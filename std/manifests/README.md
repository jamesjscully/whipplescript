...
- the CLI registers them as `EMBEDDED_STD_MANIFESTS` (contract registration —
  `use std.memory` / `use std.messaging` / `use std.script` resolve with no
  package lock), and
- the parser's build script generates the `EFFECT_OPERATION_GRAMMAR` table from
  ...
  `curate`, and `send`. `script.json` is deliberately absent from that build
  list: it carries no constructs/grammar (`exec` is core grammar); it exists
  for package identity and the `script.raw` capability row
  (spec/std-script.md, hard-off Layer 2).
