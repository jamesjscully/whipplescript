...
Status: active execution tracker for usable Codex, Claude, and Pi provider
support.
...
Build the shared provider capability and configuration layer first. Codex,
Claude, and Pi have different native shapes, so the shared layer must describe
...
provider-generated attachment coverage beyond fixed artifact fixtures. Codex,
Claude, and Pi source-workflow bridge smokes now pass live and include
store-reopen replay/recovery no-op checks through `whip recover`.
Pi native terminal evidence now has deterministic restart recovery coverage for
the crash window after a Pi RPC terminal event is persisted but before the
WhippleScript terminal event append. Native provider launch failures now route
through the native boundary path, so an unavailable Pi/Codex/Claude command
...
Codex app-server, Claude Agent SDK, and Pi RPC endpoint/session probes into a
...
Codex, Claude, and Pi when the workflow bridge requests a read-only workspace.
Live Codex, Claude, and Pi fixed-file artifact fixtures now create artifacts in
...
end as `error_max_turns`, and Pi no-tools write probes complete with a normal
assistant stop, so these surfaces do not expose a stable provider-emitted
...
- Treat Codex app-server schema, Claude Agent SDK semantics, and Pi SDK/RPC
  semantics as separate adapter surfaces.
...
| --- | --- | --- | --- |
| NP-000 Surface validation | [x] | Confirm supported Codex, Claude, and Pi integration surfaces. | `scripts/check-native-provider-surfaces.sh` passes and `spec/native-provider-surfaces.md` records the assumptions. |
...
| NP-040 Pi native spike | [x] | Start a Pi SDK or RPC session and record session/event metadata. | Isolated read-only smoke records session id, event stream entries, provider/model selection, tool policy, redacted evidence, validated abort behavior, live RPC error shape, and live source workflow bridge execution. |
| NP-050 Lifecycle normalization | [x] | Normalize native provider events into durable `agent.turn.*` events/facts. | Kernel and CLI tests cover Codex/Claude/Pi event normalization, durable lifecycle events/facts, native adapter event bridging into run terminal outcomes/artifact rows, redacted native-event evidence, and status/runs/trace exposure. |
| NP-060 Provider-native cancellation | [x] | Implement capability-declared cancellation per provider. | Cancellation request, acknowledgement, late completion, timeout, unsupported-cancellation, Codex/Pi native stop, and Claude live interrupt-normalization tests pass without duplicate terminal outcomes. |
| NP-070 Artifacts and evidence | [~] | Capture artifacts, diffs, transcripts, stream chunks, and capture failures. | Required artifact capture failure prevents false success; redaction fixtures prove no secrets leak; Codex live diff artifact capture and Claude/Pi live fixed-file fixture capture are validated, with broader provider-generated attachment coverage still open. |
...
| Pi | Native adapter should target SDK or `pi --mode rpc`, not ordinary print mode. | [x] | Pi SDK/RPC docs and local CLI shape captured in `spec/native-provider-surfaces.md`. |
| Pi | Session ids and event streams are available for durable correlation. | [x] | Live RPC `get_state` records session/model metadata; live in-flight abort smoke observes `agent_start`, `turn_start`, assistant message events, one `turn_end`, `agent_end`, and abort acknowledgement. |
| Pi | Cancellation maps to RPC `abort` for the current operation. | [x] | Live RPC smoke with tools disabled validates prompt acceptance, abort acknowledgement, assistant `stopReason: "aborted"`, and exactly one `turn_end`. |

## NP-010 Capability And Config Layer

| Task | Status | Description | Validation |
| --- | --- | --- | --- |
| NP-011 | [x] | Add provider capability structs for surface kind, protocol/schema version, session identity fields, stream event kinds, tool policy shape, cancellation depth, artifact support, health checks, and auth requirements. | Kernel provider tests cover built-in Codex, Claude, Pi, and fixture capabilities. |
...
| NP-015 | [x] | Teach release readiness strict mode to require native capability validation, not just command availability. | `scripts/check-native-provider-configs.sh` validates Codex, Claude, and Pi native bindings; release readiness runs it with strict mode tied to `WHIPPLESCRIPT_RELEASE_STRICT_EXTERNAL`. |
...
## NP-040 Pi Native Spike

| Task | Status | Description | Validation |
| --- | --- | --- | --- |
| NP-041 | [x] | Choose Pi SDK sidecar vs `pi --mode rpc` as the first adapter surface. | `spec/pi-rpc-strategy.md` chooses `pi --mode rpc`; `scripts/check-pi-rpc-surface.sh` validates local CLI/package posture and a live offline RPC `get_state` response. |
| NP-042 | [x] | Add minimal Pi SDK/RPC client that streams one read-only turn. | `pi_rpc` deterministic fake tests cover typed request/response matching, event buffering, `get_state` session/model extraction, malformed JSON, remote error, unexpected response, timeout, and redacted event counting. |
| NP-043 | [x] | Map WhippleScript profiles/capabilities to Pi tools, extensions, skills, model/provider, and resource loading. | `pi_rpc` policy tests cover read-only mapping, writer/edit/bash mapping, CLI arg generation, profile denial, forbidden workspace, missing approval, invalid provider/model, and invalid extension refs. |
| NP-044 | [x] | Capture Pi session id, parent id, event stream entries, tool events, model/provider selection, and terminal result. | Kernel evidence summary redacts event/terminal payloads while preserving session/model/count/terminal shape; store helper records `pi.rpc.evidence` with provider/session/run links and reopen coverage; deterministic adapter coverage proves remote prompt errors map to redacted native boundary failures without raw provider messages. |
| NP-045 | [x] | Validate Pi cancellation controls before adding support. | `WHIPPLESCRIPT_PI_RPC_INTERRUPT_LIVE=1 scripts/check-pi-rpc-interrupt-smoke.sh` validates live in-flight prompt cancellation with `stopReason: "aborted"` and one `turn_end`; default mode still validates the offline abort command shape. |
| NP-046 | [x] | Route source workflow effects to the Pi RPC native bridge. | A live no-tools RPC prompt was validated before plumbing; `whip worker/dev --provider pi` builds a read-only Pi RPC turn request, launches `pi --mode rpc --no-session`, and `WHIPPLESCRIPT_PI_NATIVE_WORKFLOW_LIVE=1 scripts/check-pi-native-workflow-smoke.sh` passes with Pi RPC lifecycle events recorded durably. |

## NP-050 Lifecycle Normalization

| Task | Status | Description | Validation |
| --- | --- | --- | --- |
| NP-051 | [x] | Define canonical native lifecycle event names and payload fields. | `native_lifecycle` maps Codex, Claude, and Pi provider events into canonical `agent.turn.started`, `agent.turn.streamed`, `agent.turn.tool_requested`, `agent.turn.artifact_captured`, and terminal event names with redacted payload shapes. |
| NP-052 | [x] | Derive `agent.turn.*` facts from native provider events. | `RuntimeKernel::record_native_agent_turn_observation` appends the canonical lifecycle event and derives a same-name fact; `RuntimeKernel::run_native_agent_turn` bridges native adapter events into durable runs, artifact rows, and terminal outcomes; kernel tests cover both a Pi aborted `turn_end` becoming `agent.turn.cancelled` without raw payload leakage and a fake native adapter completing through the bridge. |
...
| Task | Status | Description | Validation |
| --- | --- | --- | --- |
| NP-061 | [x] | Model cancellation depth in provider capabilities and binding policy. | Provider capability/config tests cover validated Pi `native_stop`, Codex `native_stop`, Claude cooperative/fake-only posture, and unsupported-depth failures before launch. |
| NP-062 | [x] | Add cancellation acknowledgement states to store/trace if current request-only model is insufficient. | Current store request model is sufficient for validated Codex/Pi paths: terminal completion marks requests `terminal`, and worker metadata records provider acknowledgement order; no new trace state needed yet. |
| NP-063 | [x] | Implement provider-specific cancellation for Codex, Claude, and Pi only after each behavior is validated. | Worker cancellation policy distinguishes Codex/fixture ack-before-terminal and Pi ack-after-terminal-allowed; Claude sidecar live interrupt is validated through local Claude auth and normalized to cancellation after the provider emits `error_during_execution`. |
| NP-064 | [x] | Handle late completions after cancellation request/acknowledgement by explicit policy. | Tests cover unsupported-provider late completion after cancel request, supported-provider acknowledgement, Pi ack-after-terminal metadata, duplicate worker acknowledgement suppression, timeout-after-cancellation request resolution, duplicate terminal rejection, and rejection of cancellation requests after terminal completion. |
...
| NP-072 | [~] | Capture provider diffs, changed files, attachments, command/tool output refs, and transcript refs. | Command-backed providers now emit transcript/stdout/stderr reference artifacts into the canonical manifest; Codex live artifact smoke validates diff notifications for a provider-created file without storing patch bodies; Claude and Pi live artifact fixture modes pass with disposable acknowledgements and record only metadata/hash. The Claude live report also records redacted content-block/tool-use counts so completed-without-file responses cannot masquerade as artifact success. Remaining gap is provider-generated attachments beyond fixed file/diff fixtures. |
...
| NP-103 | [x] | Update release checklist with strict native-provider ship gates. | `spec/release-checklist.md` now has a native-provider release gate with exact Codex/Claude/Pi commands, strict real-provider validation command, disposable target requirement, workflow dispatch requirement, and required report artifacts. |
...
| Date | Entry |
| --- | --- |
| 2026-05-31 | Validated local Codex, Claude, and Pi surfaces and added `scripts/check-native-provider-surfaces.sh`. |
...
| 2026-06-01 | Chose Pi RPC subprocess as the first native Pi adapter surface and added a local/package/RPC surface report check. |
| 2026-06-01 | Added the minimal Pi RPC JSONL client with deterministic fake tests for state metadata, events, and error handling. |
| 2026-06-01 | Added Pi RPC profile/capability to tool/provider/model/extension/skill policy mapping with denial tests. |
| 2026-06-01 | Added redacted Pi RPC event summaries and durable store evidence refs for session/run evidence. |
| 2026-06-01 | Validated Pi RPC `abort` command response shape offline and added a client abort method. |
| 2026-06-01 | Validated live in-flight Pi RPC abort ordering with tools disabled: prompt accepted, abort acknowledged, assistant stop reason `aborted`, and exactly one `turn_end`. |
| 2026-06-01 | Added native lifecycle normalization for Codex, Claude, and Pi provider event shapes and store event/fact recording for canonical `agent.turn.*` observations. |
...
| 2026-06-01 | Added provider-native cancellation policy classification for Codex/fixture and Pi acknowledgement ordering; Claude remains unsupported until live Agent SDK cancellation validation. |
...
| 2026-06-01 | Added native strict mode to real-provider validation so command-wrapper-only selections cannot satisfy Codex/Claude/Pi native-provider gates. |
...
| 2026-06-02 | Added optional native-provider CI matrix for Codex, Claude, and Pi, including report artifact uploads and strict dispatch gating. |
...
| 2026-06-02 | Added non-live `whip doctor --providers` health posture for Codex, Claude, and Pi CLI/credential checks without starting provider turns or exposing secret values. |
...
| 2026-06-02 | Added Codex app-server profile/capability policy mapping with denial tests, matching Claude and Pi approval/workspace boundary coverage. |
...
| 2026-06-02 | Validated a live Pi RPC no-tools prompt, wired `whip worker/dev --provider pi` through the native bridge for read-only source workflow turns, and added a live source workflow -> Pi native smoke. |
| 2026-06-02 | Added `whip recover <instance>` and extended Codex, Claude, and Pi native workflow smokes with store-reopen replay/recovery checks that assert no duplicate terminal recovery after live provider completion. |
...
| 2026-06-02 | Extended the source workflow native policy-denial regression to Codex, Claude, and Pi. The same read-only bridge plus source-required `repo.write` effect now proves all three native adapters fail durably with `whip.native.boundary_error.workspace_denied`. |
...
| 2026-06-02 | Added Claude Agent SDK and Pi RPC remote-error adapter coverage proving sidecar/RPC remote failures during start or prompt submission map to provider-specific recoverable native boundary failures without leaking raw provider messages into redacted evidence shapes. |
| 2026-06-02 | Added `scripts/check-claude-agent-sdk-error-smoke.sh` and `scripts/check-pi-rpc-error-smoke.sh`, wired both into real-provider native mode and release readiness, and validated live Claude SDK invalid-executable and Pi RPC invalid-command error shapes with no raw error payload capture. |
...
| 2026-06-02 | Added a Pi RPC native adapter boundary that captures state/session/model metadata, starts prompts, normalizes RPC lifecycle events, and sends non-terminal native `abort` acknowledgements. |
...
scripts/check-pi-native-adapter.sh
