...
| `lease.renew` verb | TTL sizing suffices for shipped holds; `renew` privilege is reserved to std.tracker and a second renewal vocabulary should not be invented independently | rides the std.tracker renew design (ADR-0002 noun work) so both surfaces share one renewal contract |
