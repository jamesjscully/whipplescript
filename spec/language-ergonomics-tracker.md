...
external scripts and agents. LLM-driven control flow goes through BAML-typed
...
| 14 | External event ingress ([C5](#c5-external-event-ingress--decided-2026-06-10)) | **done** | **done** ([event-ingress.md](event-ingress.md)) | **done** (3a: `event_*` e2e in `soft_middle.rs`: notify→typed fact→reaction, boundary rejection of malformed/undeclared, typed-field + undeclared-event checks) | **done** (3a: `event` decl, bare `when <event>` typed reaction, `whip notify` CLI; 3b hosted webhooks deferred per spec) | **done** (review verified boundary rejection cannot land ill-typed facts; liveness lint exempts dotted events naturally) |
...
| 16 | Messaging: durable tuple-space ([C7](#c7-messaging-a-durable-tuple-space--decided-2026-06-10)) | **done** | **done** ([coordination.md](coordination.md), [event-ingress.md](event-ingress.md)) | **done** (cross-instance notify e2e: sender effect validates payload against the declared event shape and lands the typed fact in the peer) | **done** (`notify <instance> event <name> { payload }` effect; ledger pub-sub via C6; `has entry for` projection sugar is the noted follow-up) | **done** (review verified payload validation failure fails the notify effect without touching the peer) |
...
usable `exec` stdout and is the shared parser behind 14's event payloads;
then 13 (absolute time, extends the A2 worker pass); then 14a (event
...
`notify` effect + mailbox pattern (16) compose with 14. 16 needs no
...
- [x] **A4a.** Inline anonymous coercion:
      ...
      names (same visibility discipline as flow lowering). Same effect kind
      (`baml.coerce`), same `after` branching. Rationale: every point of
      friction on the typed-decision path is an incentive to parse prose
...
that deliberately routes computation to BAML (semantic) and `exec` (arbitrary)?
...
- [x] **C1b.** The discriminant is **synthesized from the variant name** into
      ...
      name); no author-side typo or WS↔BAML desync possible. Not `$variant`
...
- [x] **C1d.** Runtime is internally-tagged JSON (`{"variant":"Approved",
      ...}`); WS `case` compares the tag exactly because BAML's parser has
      already canonicalized it.
- [x] **C1e.** BAML mapping settled by SAP source evidence, not assumption: a
      sum type is a BAML union of classes; SAP scores each variant and the
      ...
      coerce failure routed to `after <coerce> fails`. WS generates no BAML
      source — it references the user-written BAML type and `check`
...
- **General/structural pattern matching** — declined beyond C1. Targeted
  ...
  features.
- **Regex / string manipulation** — declined. Text→structure is BAML's job
...
- [x] **C4c.** Out of scope v1: recurrence/cron (exact cron needs the rejected
      daemon; recurring needs use an external scheduler + `whip notify`, C5);
      absolute `timeout` on effects (`timeout` stays relative-only).

### C5. External event ingress — DECIDED 2026-06-10

Pass 2. Spec: [`event-ingress.md`](event-ingress.md). An authenticated
external event becoming a durable typed fact is the event-sourced core
generalized (the inbox, generalized). Split at the language/operations seam.

- [x] **C5a (3a, core).** `event <dotted.name> { schema }` declares a typed
      external event (class-body grammar). It is the typed ingress manifest and
      subsumes the `@external` liveness escape for declared events.
- [x] **C5b (3a, core).** Reaction reuses `when` — a declared event gets the
      typed bare form `when deploy.finished as d`; undeclared dotted facts keep
      the untyped `when fact <name>`.
- [x] **C5c (3a, core).** Injection: `whip notify <instance> --event <name>
      --data <json>`; `--data` is parsed against the event schema via the C3
      ...
      webhook and shells out to `whip notify`. Replay-safe.
- [x] **C5d (3b, opt-in ops).** Hosted webhooks are a runtime mode
      (`whip serve --webhooks`) configured, not coded: a config maps an `event`
...
- [x] **C7c.** The one thin addition: an in-workflow `notify <instance> event
      <name> { payload }` effect — C5's injection turned inward, landing a typed
      durable event in a target instance. Still "inject a durable event," not
      "open a channel"; no liveness coupling, replay-safe.
