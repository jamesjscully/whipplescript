...
**Status: CLOSED (shipped into code, 2026-07-03).** Decision-record style; decisive where groundings disagree, with assumptions stated inline and collected at the end. This began as a research thread and is retained as the historical design record now that all four families + the selector capstone have shipped and are gated. The registry row is `closed`. The only non-`[x]` item is the Stage 1a *pass-collapse* refactor, which was deliberately dropped (Rev 2026-06-28e) with no capability loss — it is a deferred-with-cause `[~]`, not open build intent. §9 remains as demand-gated / v2 design questions that live on independently of this tracker; they are not blockers to closure.

> **Revision 2026-07-03 — closed.** All stages shipped and gated (Stages 1a user-facing checks / 1b syntax break / 2 Family A / 3 Family B / 4 Family C + the selector-doctrine capstone; §5.4 observer-only construction check landed 2026-07-01). Per tracker discipline (`spec/TRACKERS.md`: a tracker holds only *open intent*), the Stage 1a box is split below — the shipped user-facing checks are `[x]` with evidence, and the dropped internal pass-collapse is recorded as a single deferred-with-cause `[~]` (why: low-value/high-risk internal tidiness, no capability lost; when: revisit only if the duplication actually bites). Nothing here is a `[ ]` any longer. The IFC-substrate phases (3/4 below) reached their v1 landings on the IFC tracks that own them (`information-flow-audit-findings.md` E5/E6, `x2-flow-signature`/DR-0030 producer-side flow signature) and are cross-referenced here for sequencing only. Remaining §9 items are demand-gated future design questions (opt-in inapplicable-sibling lint, computed/multiple discriminants v2 gate, per-field milestone typing, opt-in liveness annotation, in-arm-crossing selector diagnostic) — none is a build commitment of this tracker.
...
### 5.2 `after … fails as f` typing — PROMOTED to its own first-class record: DR-0032

This section's original proposal ("narrow `after … fails as f` to a single `TerminalFailed`") was corrected in Revision 2026-06-28c (failure payloads are effect-kind-specific — `exec` exposes `.message`, `coerce`/workflow `.reason` — so one `TerminalFailed` would regress them). The corrected, generalized design — **effect failure is a closed `EffectError` discriminated family** (common base + per-kind extras, *static* narrowing by effect kind, extras additive-behind-narrowing, redaction per variant) — is now its own first-class ADR: **`spec/decision-records/0032-typed-effect-failures.md`**. The current `"fails" => {}` (untyped `f`) remains the deliberate correct interim until DR-0032's v1 (consistent wire base + base typing) lands. The Gleam comparison (their `Result(value, error)` is bought with *generics*, which whipplescript excludes) is what reframed this from "N ad-hoc schemas" to "one no-generics discriminated family" — see DR-0032 Problem.
...
- [x] **Stage 1a — narrowing-rule user-facing checks.** SHIPPED + verified in tree: exhaustiveness lifted to **error** with the conditional-coverage diagnostic (`lib.rs:10798/10872`, tested); redundancy / arm-after-wildcard check (tested); the §5.5 soundness fix as a *targeted* check — conflicting reuse of an effect binding with different result types is rejected (`lib.rs:7671`, `rejects_conflicting_reused_effect_binding`), which closes the exact `effect_payload_types`-keying ambiguity §5.5 names. Observer-only construction check (§5.4) SHIPPED 2026-07-01: `is_observer_only_schema` rejects a rule that `record`s a terminal-family schema (`TerminalFailed`/`TerminalTimedOut`/`TerminalCancelled`) with a routing diagnostic (→ `fail <failure> {…}` / `after … fails/times out/cancels as f`); `WorkItem` and other user-writable builtins are unaffected (regression-guarded). Tests `rejects_recording_observer_only_terminal_schema` + `allows_recording_user_writable_builtin_schema` (parser lib.rs).
- [~] **Stage 1a — internal pass-collapse refactor: DEFERRED-WITH-CAUSE, dropped (Rev 2026-06-28e).** *Why:* it buys **no capability** — Families A and B shipped without it — only internal tidiness, at regression risk to working core machinery. *When:* revisit only if the duplication actually bites. Not built: the unified `validate_and_lower_cases` pass + `Family` trait + single `branch_scope` site (the three passes `validate_case_blocks`/`collect_rule_case_metadata`/`collect_terminal_case_metadata` + `terminal_case_tags`/`finite_case_domain` still stand); `after`→`case` desugar + textual-scanner deletion (scanner still at `lib.rs:8356`; §8 corner 7 in fact **decided to keep** `after` as a surface form); the *blanket* `duplicate_binding_in_scope` (only the targeted conflicting-reuse form shipped). No user-facing behavior gap. This is the sole non-`[x]` item and is not open build intent.
- [x] **Stage 1b — syntax break to `Tag as binding`.** SHIPPED: space-form parser path deleted (`lib.rs:8118/:10735`), corpus migrated (by hand — the `whip fmt --upgrade-as-bindings` helper was **not** built; no back-compat means the one-time manual migration sufficed), spec/sum-types.md §2.1 + language.md §B.2.2 updated. Deferrable/reconsiderable independently of 1a.
...
**DECISION 2026-06-29 (Jack) — signal-trigger sources: way-station + destination.** Grounding the "are signals low-integrity" question against the checker corrected its premise: **integrity is *not* hardcoded by construct kind.** `integrity_authority(resource)` (ifc.rs:442) looks the resource up in the envelope's `integrity` map and defaults to `public` (untrusted bottom) only when unlabeled; `injects` uses that lookup. So `message from <internet_webhook>` and `message from <internal_bus>` already get *different* integrity if governance labels them differently — the determination is already per-resource at the governance layer, which is exactly where it belongs (it requires knowledge outside the language boundary). The genuine wart is narrower: **source *recognition* is hardcoded by kind** (only `message from` / `human answered` are recognized as inbound read-sources; a `when <Signal> as e` trigger is recognized as *nothing*). So a signal isn't "hardcoded safe" — it is **hardcoded invisible**: under a governed envelope an untrusted signal driving a trusted sink is *not caught at all*. That is a fail-**open** gap, not a conservative default.

The fix follows the no-kind-intrinsic-integrity principle by **removing** a special-case rather than adding one: make source recognition uniform — a `when <Signal> as e` trigger is a tracked read of a governed resource whose integrity is envelope-declared, defaulting `public`/low (fail-closed), identical to how channels already work. Two-stage plan:

- **Way-station — signal-name-as-resource (near-term, this is the small slice). SHIPPED 2026-06-30.** The integrity-bearing resource is the signal name (`signal:<name>`); governance labels it, default low. This closes the fail-open gap and makes the §5.6 channel-2 selector check live (a Family-B signal discriminant gating a crossing is now a recognized low-integrity selector). Landed in `ifc.rs`: `when <Signal> as e` is recognized (via `ir.events` membership) as a `signal:<name>` read source in `check_with_envelope` (leak + inject, both axes), `ifc_surface`, `check_principal_ceiling`, and the `low_integrity_bindings` selector collection (gated on the signal being unvouched — `integrity_set("signal:<name>")` empty — so a vouched `signal:<name> from <Role>` may steer a crossing). 4 unit tests + CLI run; recognition is now uniform across channels / human answers / signals. **Caveat to pin (still true):** an *unlabeled* signal defaults low, so internal peer signals also default low and trip injection into trusted sinks until vouched — temporarily *more* classification than the destination wants. A signal-name label is a coarse proxy for "this signal only ever arrives from trusted emitters." **Do not let anything subtle depend on per-signal-name integrity labels** — they are scaffolding the destination tears out.
- **Destination — emitter-carried integrity. SHIPPED 2026-06-30.** A signal marked `internal` derives its integrity from its emitters (`derived_signal_integrity` in ifc.rs: each `emit signal X` carries the intersection of its emitting rule's read-source vouchers; `when X` reads the meet over emitters), spanning packages (an imported `@tool`'s emit folds into the consumer's `signal:X`, computed under the consumer envelope from the pinned source via `check_with_envelope_imports`/`imported_tool_irs` — no producer label attestation needed). `whip signal` refuses external injection of an internal signal (no laundering). Modeled in `infoflow-signal-carriage.maude` (3 cov / 2 bite). So only external entry points are hand-classified; internal flows propagate automatically. Original reasoning: A signal carries the emitting instance's integrity across the boundary; the receiver's `when <Signal>` reads the carried label, so only information entering from an *external* source is ever hand-classified and internal flows propagate automatically. Rationale (the load-bearing argument, not aesthetics): this is the only model where the labeling burden stays `O(external entry points)` and *fixed* under maintenance — per-internal-hop classification is `O(every flow)` and fails in the leaking direction (a forgotten re-label silently inherits a trusted default). `emit signal X to <peer>` carrying the emitter's label **is** an output port labeled at the emitter's boundary, read by the receiver — i.e., W6 label carriage + the DR-0029 cross-package boundary label. **This is owned by the W6/DR-0029 carriage track, not by discriminated families.** See `information-flow-audit-findings.md`.
