...
**Implementation-scope finding (2026-06-28, from starting the build).** The runtime *delivery* is ~90% reuse, but the *surface* cascades wider than that estimate implied — the build is a multi-hour vertical, not a quick increment. Concretely: `emit milestone` is a new `BodyEffectKind` → a new `IrEffectKind::Milestone` → **~15-20 exhaustive `IrEffectKind` match sites** (effect-kind string, terminal-payload, capability/construct-catalog mappings, the runtime effect dispatch); `after p reaches "<name>"` adds an `AfterPredicate::Reaches` (keep it fieldless/`Copy` — put the name on `AfterBlock.milestone`, as prototyped) → a new dependency-predicate + its lowering; then the core is the deep extension of the ~360-line `run_workflow_invoke_effect` (read child milestone facts, derive idempotent `workflow.invoke.reached:<name>` facts) plus a child-side milestone-fact handler and a per-child milestone registry for `tags()`/exhaustiveness. **Recommendation:** build it as one focused dedicated pass (parser → IR-with-cascade → runtime → e2e together, so the syntax isn't exposed before delivery works), not interleaved — the parser layer was prototyped and reverted to keep the tree green precisely because a half-built effect-kind cascade leaves the runtime broken. The `milestone-signal.maude` model already gates the contract.
