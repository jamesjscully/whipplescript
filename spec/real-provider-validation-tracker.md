...
| Beyond command-wrapper smoke | [x] | Integration maintainer | `scripts/check-real-providers.sh` can select native Codex, Claude, and Pi adapter suites and fails when only compatibility command wrappers are configured for shipping mode. | `WHIPPLESCRIPT_REAL_PROVIDER_NATIVE_STRICT=1` defaults to Codex/Claude/Pi, rejects coerce command-wrapper selections, requires native provider configs, and runs native surface probes. |
