...
and Claude turns through supported native surfaces, preserve provider
...
| Provider lifecycle normalization | [x] | Runtime maintainer | Native provider events normalize into `agent.turn.started`, `agent.turn.streamed`, `agent.turn.tool_requested`, `agent.turn.artifact_captured`, `agent.turn.completed`, `agent.turn.failed`, `agent.turn.timed_out`, and `agent.turn.cancelled` facts/events. | `native_lifecycle` maps Codex and Claude events into canonical lifecycle kinds; store/kernel tests record redacted native-event evidence and expose lifecycle through status/runs/trace/evidence JSON. |
...
| Provider-native cancellation requests | [x] | Provider maintainers | Codex and Claude adapters map WhippleScript cancellation requests to native provider stop/abort/session APIs when supported. | Codex interrupt smokes exist; Claude sidecar uses SDK `Query.interrupt()` before `AbortController` and live local-auth smoke validates that provider `error_during_execution` after interrupt is normalized to one `claude.turn.cancelled` acknowledgement. |
