...
| dynamic-tasks | done | `armature-dynamic-tasks` | Codex | Added ephemeral dynamic task definitions; checks: `cargo test -p armature-cli --test e2e dynamic_task_event_and_watch_lifecycle`; `cargo test -p armature-daemon`; `cargo test`. |
| sdk-docs | done | `armature-dynamic-sdk-docs` | Codex | Added thin dynamic-management SDK namespaces and docs. Pushed branch; integrated into primary checkout; checks: `npm test --workspace @armature/sdk`; `cargo test`; `cargo clippy --all-targets -- -D warnings`. |
...
Checks: `cargo test -p armature-cli --test e2e
dynamic_task_event_and_watch_lifecycle`; `cargo test -p armature-daemon`;
`cargo test`.

### sdk-docs

Done. Added CLI-backed TypeScript SDK namespaces for `task`, `run`, `event`,
...
notes. Preserved the no-second-runtime boundary. Pushed
`origin/armature-dynamic-sdk-docs` and integrated it into the primary
checkout. Checks in primary: `npm test --workspace @armature/sdk`; `cargo
