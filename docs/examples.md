...
(Codex + Claude) and `incident-router` (routing across `codex`/`claude`
`AgentRef`s at check-time). The catalog commands are verified
...
| [`incident-router.whip`](https://github.com/jamesjscully/whipplescript/blob/main/examples/incident-router.whip) | `whip check examples/incident-router.whip` | Rich guards and dynamic routing: arrays, maps, optionals, `exists`, `in`, assertions, `AgentRef`. **Optional-provider exhibit**: `AgentRef` routing across `codex`/`claude` at check-time (Codex and Claude are the usable delegating providers). |
