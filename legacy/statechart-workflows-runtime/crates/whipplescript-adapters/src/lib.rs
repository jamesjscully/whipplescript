...
pub struct CapabilityPolicyDocument {
    ...
    pub mode: whipplescript_workflow::policy::PolicyMode,
    #[serde(default)]
...
fn default_policy_mode() -> whipplescript_workflow::policy::PolicyMode {
    whipplescript_workflow::policy::PolicyMode::Local
}
...
pub struct AdapterManifest {
    ...
    pub version: String,
    #[serde(default)]
    pub types: BTreeMap<String, whipplescript_workflow::schema::Schema>,
    #[serde(default)]
    pub effects: BTreeMap<String, AdapterEffect>,
    #[serde(default)]
    pub events: BTreeMap<String, whipplescript_workflow::schema::Schema>,
}

impl AdapterManifest {
    pub fn diagnostics(&self) -> Vec<whipplescript_workflow::Diagnostic> {
...
pub struct AdapterEffect {
    pub category: whipplescript_engine::effects::EffectCategory,
    #[serde(default)]
    pub required_capabilities: Vec<String>,
    pub input: whipplescript_workflow::schema::Schema,
    pub output: whipplescript_workflow::schema::Schema,
    pub idempotent: bool,
    #[serde(default)]
...
pub fn json_plan_adapter_manifest() -> AdapterManifest {
    ...
    effects.insert(
        ...
        AdapterEffect {
            category: whipplescript_engine::effects::EffectCategory::SyncValue,
            required_capabilities: vec!["resource.plan.read".to_string()],
            input: whipplescript_workflow::schema::Schema::Json,
            output: whipplescript_workflow::schema::Schema::String,
            idempotent: true,
    ...
    effects.insert(
        ...
        AdapterEffect {
            ...
            required_capabilities: vec!["resource.plan.read".to_string()],
            input: whipplescript_workflow::schema::Schema::Json,
            output: whipplescript_workflow::schema::Schema::Int,
            idempotent: true,
            failure_categories: vec!["resource_unavailable".to_string()],
            model: Some(AdapterModel::Deterministic),
        },
    ...
    effects.insert(
        ...
        AdapterEffect {
            ...
            required_capabilities: vec!["resource.plan.read".to_string()],
            input: whipplescript_workflow::schema::Schema::Json,
            output: whipplescript_workflow::schema::Schema::Json,
            idempotent: true,
            failure_categories: vec!["resource_unavailable".to_string()],
            model: Some(AdapterModel::Deterministic),
        },
...
pub fn json_human_review_response_event_schema() -> whipplescript_workflow::schema::Schema {
    whipplescript_workflow::schema::Schema::Record {
        fields: vec![
            whipplescript_workflow::schema::Field {
                name: "reviewId".to_string(),
                schema: whipplescript_workflow::schema::Schema::String,
            },
            whipplescript_workflow::schema::Field {
            ...
            whipplescript_workflow::schema::Field {
                ...
                schema: whipplescript_workflow::schema::Schema::Optional {
                    inner: Box::new(whipplescript_workflow::schema::Schema::String),
                },
            },
        ],
    }
...
pub fn json_human_review_adapter_manifest() -> AdapterManifest {
    ...
    effects.insert(
        "askHuman".to_string(),
        AdapterEffect {
            category: whipplescript_engine::effects::EffectCategory::HumanObligation,
...
pub fn json_agent_adapter_manifest() -> AdapterManifest {
    ...
    effects.insert(
        "start".to_string(),
        AdapterEffect {
            category: whipplescript_engine::effects::EffectCategory::AsyncInvocation,
    ...
    effects.insert(
        "send".to_string(),
        AdapterEffect {
            category: whipplescript_engine::effects::EffectCategory::Message,
...
pub fn json_agent_finished_event_schema() -> whipplescript_workflow::schema::Schema {
    whipplescript_workflow::schema::Schema::Record {
        fields: vec![
            whipplescript_workflow::schema::Field {
                name: "id".to_string(),
            ...
            whipplescript_workflow::schema::Field {
                ...
                schema: whipplescript_workflow::schema::Schema::Optional {
                    inner: Box::new(whipplescript_workflow::schema::Schema::Int),
                },
            },
        ],
    }
...
) -> Result<(), whipplescript_engine::effects::EffectError> {
    let review_id = payload
        .get("reviewId")
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| {
            whipplescript_engine::effects::EffectError::Unsupported(
    ...
    for manifest in manifests {
        ...
        for (effect_name, effect) in &manifest.effects {
            ...
            if !effect.idempotent
                && matches!(
                    effect.category,
                    whipplescript_engine::effects::EffectCategory::AsyncInvocation
                        | whipplescript_engine::effects::EffectCategory::Message
                        | whipplescript_engine::effects::EffectCategory::HumanObligation
                )
            {
                diagnostics.push(error(format!(
...
pub fn validate_baml_http_policy(
    ...
    let strictest_mode = policies
        ...
        .unwrap_or(whipplescript_workflow::policy::PolicyMode::Local);
    ...
    if policies.iter().any(|policy| {
        ...
        let severity = if unknown_capability_is_error(
            strictest_mode,
            whipplescript_engine::effects::EffectCategory::SyncValue,
            "baml.coerce",
        ) {
            whipplescript_workflow::Severity::Error
        } else {
            whipplescript_workflow::Severity::Warning
        };
        ...
        .any(|policy| policy.allow_baml_network == Some(false))
    {
        ...
        && strictest_mode == whipplescript_workflow::policy::PolicyMode::Enterprise
    {
        diagnostics.push(error(
    ...
    } else if strictest_mode == whipplescript_workflow::policy::PolicyMode::Enterprise {
        diagnostics.push(error(format!(
...
fn validate_type_cycles(
    ...
    manifest_name: &str,
    types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
) {
...
fn collect_schema_refs<'a>(schema: &'a whipplescript_workflow::schema::Schema, refs: &mut Vec<&'a str>) {
    match schema {
        whipplescript_workflow::schema::Schema::Ref { name } => refs.push(name),
...
struct RegisteredEffect<'a> {
    manifest_name: &'a str,
    types: &'a BTreeMap<String, whipplescript_workflow::schema::Schema>,
...
fn validate_state_effects(
    ...
    ir: &whipplescript_workflow::WorkflowIr,
    state_name: &str,
    state: &whipplescript_workflow::ir::State,
) {
...
fn validate_invariant_effects(
    ...
    for invariant in &ir.invariants {
        let whipplescript_workflow::ir::Invariant::Expression { name, expr, span } = invariant else {
            continue;
        };
...
fn validate_steps_effects(
    ...
    for step in steps {
        ...
        for value in step.args.values() {
            if let Ok(expr) = serde_json::from_value::<whipplescript_workflow::expr::Expr>(value.clone())
            {
                validate_expr_effects(diagnostics, registry, ir, owner, &expr, step.span.as_ref());
            } else if let Ok(exprs) =
                serde_json::from_value::<Vec<whipplescript_workflow::expr::Expr>>(value.clone())
            {
                for expr in &exprs {
                    validate_expr_effects(
                        diagnostics,
    ...
    for step in steps {
        ...
        match step.effect.as_str() {
            "send" => validate_step_expr_schema(
                ...
                StepExprExpectation {
                    arg: "message",
                    expected: workflow_schema(whipplescript_workflow::schema::Schema::String, ir),
                },
            ),
...
struct PreciseAdapterEffectRequestShape {
    effect_name: String,
    category: whipplescript_engine::effects::EffectCategory,
    input: TypedSchema,
}
            ...
            let call_arg_schema = step
                ...
                .map(|values| {
                    infer_list_item_schema(values.iter().filter_map(|value| {
                        serde_json::from_value::<whipplescript_workflow::expr::Expr>(value.clone())
                            .ok()
...
struct BoolExprExpectation<'a> {
    owner: &'a str,
    label: &'a str,
    span: Option<&'a whipplescript_workflow::SourceSpan>,
...
fn validate_bool_adapter_expr(
    ...
    registry: &EffectRegistry<'_>,
    ir: &whipplescript_workflow::WorkflowIr,
    expr: &whipplescript_workflow::expr::Expr,
    ...
    let expected = workflow_schema(whipplescript_workflow::schema::Schema::Boolean, ir);
    if !adapter_schema_accepts_schema(
        &expected.schema,
        &expected.types,
        &actual.schema,
...
fn validate_adapter_expr_constraints(
    ...
    owner: &str,
    expr: &whipplescript_workflow::expr::Expr,
    span: Option<&whipplescript_workflow::SourceSpan>,
    scope: &AdapterExprScope,
) {
    use whipplescript_workflow::expr::Expr;
    use whipplescript_workflow::schema::Schema;
...
fn validate_capability_value_call_input(
    ...
    let whipplescript_workflow::expr::Expr::Call { name, args } = expr else {
        return;
    };
    if !name
        .split_once('.')
    ...
    if registered.effect.category != whipplescript_engine::effects::EffectCategory::SyncValue {
        return;
    }
...
fn capability_value_call_input_schema(
    ...
    ir: &whipplescript_workflow::WorkflowIr,
    scope: &AdapterExprScope,
    args: &[whipplescript_workflow::expr::Expr],
) -> TypedSchema {
    use whipplescript_workflow::schema::Schema;
...
fn adapter_schema_is_orderable(schema: &whipplescript_workflow::schema::Schema) -> bool {
    match schema {
        whipplescript_workflow::schema::Schema::String
        | whipplescript_workflow::schema::Schema::Time
        | whipplescript_workflow::schema::Schema::Duration
        | whipplescript_workflow::schema::Schema::Int
        | whipplescript_workflow::schema::Schema::Float
        | whipplescript_workflow::schema::Schema::Json => true,
        whipplescript_workflow::schema::Schema::Literal { value } => {
            value.is_string() || value.is_number()
        }
        whipplescript_workflow::schema::Schema::Union { variants } => {
            variants.iter().all(adapter_schema_is_orderable)
        }
        whipplescript_workflow::schema::Schema::Optional { inner } => adapter_schema_is_orderable(inner),
        _ => false,
    }
}
...
fn step_expr(
    step: &whipplescript_workflow::ir::Step,
    arg: &str,
) -> Option<whipplescript_workflow::expr::Expr> {
    step.args
        .get(arg)
...
fn materialize_schema_refs(
    ...
    types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
    depth: usize,
) -> whipplescript_workflow::schema::Schema {
    use whipplescript_workflow::schema::Schema;
    ...
    match schema {
        ...
        Schema::Record { fields } => Schema::Record {
            fields: fields
                .iter()
                .map(|field| whipplescript_workflow::schema::Field {
...
fn infer_literal_schema(value: &serde_json::Value) -> whipplescript_workflow::schema::Schema {
    use whipplescript_workflow::schema::Schema;
    ...
    match value {
        ...
        serde_json::Value::Object(entries) => Schema::Record {
            fields: entries
                .iter()
                .map(|(name, value)| whipplescript_workflow::schema::Field {
                    name: name.clone(),
...
fn infer_list_item_schema(
    schemas: impl IntoIterator<Item = whipplescript_workflow::schema::Schema>,
) -> whipplescript_workflow::schema::Schema {
    let mut schemas = schemas.into_iter();
    let Some(first) = schemas.next() else {
        return whipplescript_workflow::schema::Schema::Json;
    };
...
fn resolve_typed_schema_path_inner(
    ...
    nested_path: &str,
    depth: usize,
) -> Option<whipplescript_workflow::schema::Schema> {
    if nested_path.is_empty() {
        return Some(schema.clone());
    }
    if depth > 16 {
        return Some(whipplescript_workflow::schema::Schema::Json);
    }
    ...
    match schema {
        ...
        whipplescript_workflow::schema::Schema::Ref { name } => types.get(name).and_then(|schema| {
            resolve_typed_schema_path_inner(schema, types, nested_path, depth + 1)
        }),
        whipplescript_workflow::schema::Schema::Json | whipplescript_workflow::schema::Schema::Map { .. } => {
            Some(whipplescript_workflow::schema::Schema::Json)
        }
        whipplescript_workflow::schema::Schema::Record { fields } => {
        ...
        whipplescript_workflow::schema::Schema::Union { variants } => {
            ...
            match resolved.len() {
                0 => None,
                1 => resolved.pop(),
                _ => Some(whipplescript_workflow::schema::Schema::Union { variants: resolved }),
            }
        }
        _ => None,
    }
...
fn is_native_agent_step(
    ...
    step.args
        ...
        .is_some_and(|agent| {
            !matches!(
                agent.target,
                whipplescript_workflow::ir::AgentTarget::Adapter { .. }
            )
        })
}
...
fn policy_mode_rank(mode: &whipplescript_workflow::policy::PolicyMode) -> u8 {
    match mode {
        whipplescript_workflow::policy::PolicyMode::Local => 0,
        whipplescript_workflow::policy::PolicyMode::Team => 1,
        whipplescript_workflow::policy::PolicyMode::Enterprise => 2,
    }
}

fn unknown_capability_is_error(
    mode: whipplescript_workflow::policy::PolicyMode,
    category: whipplescript_engine::effects::EffectCategory,
    capability: &str,
) -> bool {
    match mode {
        whipplescript_workflow::policy::PolicyMode::Local => false,
        whipplescript_workflow::policy::PolicyMode::Enterprise => true,
        whipplescript_workflow::policy::PolicyMode::Team => {
            matches!(
                category,
                whipplescript_engine::effects::EffectCategory::AsyncInvocation
                    | whipplescript_engine::effects::EffectCategory::HumanObligation
...
fn error(message: String) -> whipplescript_workflow::Diagnostic {
...
fn error_at(
    ...
    diagnostic_at(whipplescript_workflow::Severity::Error, message, span)
}

fn diagnostic_at(
    severity: whipplescript_workflow::Severity,
    message: String,
    span: Option<&whipplescript_workflow::SourceSpan>,
) -> whipplescript_workflow::Diagnostic {
    whipplescript_workflow::Diagnostic {
...
fn validate_schema_uniqueness(
    ...
    match schema {
        ...
        | whipplescript_workflow::schema::Schema::Agent
        | whipplescript_workflow::schema::Schema::Literal { .. }
        | whipplescript_workflow::schema::Schema::Ref { .. }
        | whipplescript_workflow::schema::Schema::Json => {}
    }
}
...
fn map_key_schema_is_string_compatible(
    ...
    match schema {
        whipplescript_workflow::schema::Schema::String
        | whipplescript_workflow::schema::Schema::Enum { .. } => true,
        whipplescript_workflow::schema::Schema::Literal { value } => value.is_string(),
        whipplescript_workflow::schema::Schema::Union { variants } => {
            !variants.is_empty()
                && variants
                    .iter()
        ...
        whipplescript_workflow::schema::Schema::Ref { name } => types
            .get(name)
        ...
        | whipplescript_workflow::schema::Schema::Duration
        | whipplescript_workflow::schema::Schema::Agent
        | whipplescript_workflow::schema::Schema::Optional { .. }
        | whipplescript_workflow::schema::Schema::List { .. }
        | whipplescript_workflow::schema::Schema::Set { .. }
        | whipplescript_workflow::schema::Schema::Map { .. }
        | whipplescript_workflow::schema::Schema::Record { .. }
        | whipplescript_workflow::schema::Schema::Json => false,
    }
}

fn adapter_schema_kind(schema: &whipplescript_workflow::schema::Schema) -> &'static str {
    match schema {
        whipplescript_workflow::schema::Schema::String => "string",
        whipplescript_workflow::schema::Schema::Int => "int",
        whipplescript_workflow::schema::Schema::Float => "float",
        whipplescript_workflow::schema::Schema::Boolean => "bool",
        whipplescript_workflow::schema::Schema::Null => "null",
        whipplescript_workflow::schema::Schema::Time => "time",
        whipplescript_workflow::schema::Schema::Duration => "duration",
        whipplescript_workflow::schema::Schema::Agent => "agent",
        whipplescript_workflow::schema::Schema::Literal { .. } => "literal",
        whipplescript_workflow::schema::Schema::Enum { .. } => "enum",
        whipplescript_workflow::schema::Schema::Optional { .. } => "optional",
        whipplescript_workflow::schema::Schema::List { .. } => "list",
        whipplescript_workflow::schema::Schema::Set { .. } => "set",
        whipplescript_workflow::schema::Schema::Map { .. } => "map",
        whipplescript_workflow::schema::Schema::Union { .. } => "union",
        whipplescript_workflow::schema::Schema::Record { .. } => "record",
        whipplescript_workflow::schema::Schema::Ref { .. } => "ref",
        whipplescript_workflow::schema::Schema::Json => "json",
    }
}

fn schemas_equivalent(
    left: &whipplescript_workflow::schema::Schema,
    left_types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
    right: &whipplescript_workflow::schema::Schema,
    right_types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
    depth: usize,
) -> bool {
    if depth > 16 {
        return false;
...
fn resolve_schema_ref<'a>(
    schema: &'a whipplescript_workflow::schema::Schema,
    types: &'a BTreeMap<String, whipplescript_workflow::schema::Schema>,
) -> Option<&'a whipplescript_workflow::schema::Schema> {
    match schema {
        whipplescript_workflow::schema::Schema::Ref { name } => types.get(name),
        schema => Some(schema),
    }
}

fn adapter_schema_accepts_schema(
    expected: &whipplescript_workflow::schema::Schema,
    expected_types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
    actual: &whipplescript_workflow::schema::Schema,
    actual_types: &BTreeMap<String, whipplescript_workflow::schema::Schema>,
    depth: usize,
) -> bool {
    if depth > 16 {
        return true;
...
fn adapter_schema_allows_absent(
    ...
    match resolve_schema_ref(schema, types) {
        Some(whipplescript_workflow::schema::Schema::Optional { .. }) => true,
        Some(whipplescript_workflow::schema::Schema::Union { variants }) => variants
            .iter()
...
impl whipplescript_engine::effects::EffectDispatcher for ManifestEffectDispatcher {
    fn dispatch(
        &mut self,
        request: whipplescript_engine::effects::EffectRequest,
    ) -> Result<whipplescript_engine::effects::EffectOutcome, whipplescript_engine::effects::EffectError>
    {
        let Some((manifest, effect)) = self.find_effect(&request.effect) else {
            if matches!(
                request.category,
                whipplescript_engine::effects::EffectCategory::Event
                    | whipplescript_engine::effects::EffectCategory::Timer
                    | whipplescript_engine::effects::EffectCategory::Terminal
            ) {
            ...
            return Err(whipplescript_engine::effects::EffectError::Unsupported(format!(
...
fn enforce_runtime_policy(
    ...
    for capability in &effect.required_capabilities {
        ...
        {
            return Err(whipplescript_engine::effects::EffectError::CapabilityDenied {
                message,
...
fn dispatch_json_plan_effect(
    path: &Path,
    request: &whipplescript_engine::effects::EffectRequest,
) -> Result<Option<serde_json::Value>, whipplescript_engine::effects::EffectError> {
    match request.effect.as_str() {
        "plan.snapshot" => {
            let contents = std::fs::read_to_string(path).map_err(|error| {
                whipplescript_engine::effects::EffectError::Unsupported(format!(
...
fn append_human_review_obligation(
    ...
    let Some(document_object) = document.as_object_mut() else {
        return Err(whipplescript_engine::effects::EffectError::Unsupported(
            "human review file root must be an object".to_string(),
        ));
    };
    let reviews = document_object
...
) -> Result<serde_json::Value, whipplescript_engine::effects::EffectError> {
    let _lock = JsonFileLock::acquire(path)?;
...
) -> Result<i64, whipplescript_engine::effects::EffectError> {
...
impl JsonFileLock {
    fn acquire(path: &Path) -> Result<Self, whipplescript_engine::effects::EffectError> {
...
fn read_json_plan(path: &Path) -> Result<serde_json::Value, whipplescript_engine::effects::EffectError> {
    let contents = std::fs::read_to_string(path).map_err(|error| {
        whipplescript_engine::effects::EffectError::Unsupported(format!(
            "failed to read JSON plan file `{}`: {error}",
            path.display()
...
fn plan_call_string_arg(
    request: &whipplescript_engine::effects::EffectRequest,
    index: usize,
) -> Result<String, whipplescript_engine::effects::EffectError> {
...
fn update_task_array_status(
    ...
    status: &str,
    reason: Option<&str>,
) -> Result<bool, whipplescript_engine::effects::EffectError> {
    ...
    Err(whipplescript_engine::effects::EffectError::Unsupported(format!(
...
) -> whipplescript_engine::effects::EffectOutcome {
    let status = match request.category {
        whipplescript_engine::effects::EffectCategory::AsyncInvocation
        | whipplescript_engine::effects::EffectCategory::Message
        | whipplescript_engine::effects::EffectCategory::HumanObligation
        | whipplescript_engine::effects::EffectCategory::Timer => {
            whipplescript_engine::effects::EffectOutcomeStatus::Accepted
        }
        _ => whipplescript_engine::effects::EffectOutcomeStatus::Succeeded,
    };
...
pub mod legacy {
    //! Optional compatibility adapters for old WhippleScript concepts.
    //!
...
mod tests {
    ...
    use whipplescript_engine::effects::{
    ...
    fn manifest_dispatcher_enforces_denied_policy_at_runtime() {
        let policy = CapabilityPolicyDocument {
            mode: whipplescript_workflow::policy::PolicyMode::Enterprise,
    ...
    fn manifest_dispatcher_allows_unknown_local_policy_at_runtime() {
        let policy = CapabilityPolicyDocument {
            mode: whipplescript_workflow::policy::PolicyMode::Local,
            allowed_capabilities: Vec::new(),
            denied_capabilities: Vec::new(),
            ..Default::default()
        };
    ...
    fn manifest_validation_rejects_cyclic_type_refs() {
        ...
        manifest.types.insert(
            "A".to_string(),
            Schema::Record {
                fields: vec![whipplescript_workflow::schema::Field {
                    name: "b".to_string(),
                    schema: Schema::Ref {
                        name: "B".to_string(),
                    },
...
"#;
        ...
        assert_eq!(diagnostic.severity, whipplescript_workflow::Severity::Error);
        assert!(diagnostic
            .message
...
"#;
        ...
        assert_eq!(diagnostic.severity, whipplescript_workflow::Severity::Warning);
        assert!(diagnostic
            .message
...
"#;
        ...
        let policy = CapabilityPolicyDocument {
            mode: whipplescript_workflow::policy::PolicyMode::Team,
            allowed_capabilities: Vec::new(),
            denied_capabilities: Vec::new(),
            ..Default::default()
        };
...
"#;
        let mut ir = whipplescript_workflow::parse_source(source).expect("source parses");
        ir.invariants
            .push(whipplescript_workflow::ir::Invariant::Expression {
                name: "needsPlanRead".to_string(),
                expr: whipplescript_workflow::expr::Expr::Call {
...
"#;
        ...
        ir.invariants
            .push(whipplescript_workflow::ir::Invariant::Expression {
                ...
                expr: whipplescript_workflow::expr::Expr::Eq {
                    left: Box::new(whipplescript_workflow::expr::Expr::Call {
                        name: "plan.count".to_string(),
                        args: Vec::new(),
                    }),
                    right: Box::new(whipplescript_workflow::expr::Expr::Literal {
                        value: json!("done"),
                    }),
                },
                span: None,
