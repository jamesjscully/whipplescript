...
//! static validation. Runtime execution belongs in `whippletree-engine`.
...
pub mod syntax {
    ...
    pub enum WhippletreeLanguage {}

    impl Language for WhippletreeLanguage {
    ...
    pub type SyntaxNode = rowan::SyntaxNode<WhippletreeLanguage>;
    ...
    impl SyntaxBuilder {
        ...
        pub(crate) fn start_node(&mut self, kind: SyntaxKind) {
            self.builder.start_node(WhippletreeLanguage::kind_to_raw(kind));
        }
        ...
        fn emit_through(&mut self, index: usize) {
            while self.emit_cursor <= index {
                ...
                self.builder.token(
                    WhippletreeLanguage::kind_to_raw(token.kind),
                    token.text.as_str(),
                );
                self.emit_cursor += 1;
            }
...
pub mod source {
    ...
    pub enum SourceError {
        #[error("failed to parse Whippletree source")]
