...
use whippletree_workflow::{expr::Expr, ir::State, schema::Schema, WorkflowIr};
...
fn ensure_generated_model_invariants_supported(ir: &WorkflowIr) -> Result<(), ModelgenError> {
    for invariant in &ir.invariants {
        if let whippletree_workflow::ir::Invariant::Expression { name, expr, .. } = invariant {
...
fn record_output_space(
    fields: &[whippletree_workflow::schema::Field],
    types: &BTreeMap<String, Schema>,
    depth: usize,
) -> Vec<String> {
...
fn builtin_invariant_coverage_lines(ir: &WorkflowIr) -> Vec<String> {
    ...
    for invariant in &ir.invariants {
        if let whippletree_workflow::ir::Invariant::Builtin { name, .. } = invariant {
...
fn effect_label_for_step(step: &whippletree_workflow::ir::Step) -> Option<String> {
...
fn tla_module_name(name: &str) -> String {
    let mut module_name = String::from("Whippletree_");
...
fn maude_module_name(name: &str) -> String {
    let mut module_name = String::from("WHIPPLETREE-");
    for character in name.chars() {
...
mod tests {
    ...
    fn emits_tla_model_for_minimal_workflow() {
        ...
        assert!(model.contains("---- MODULE Whippletree_Minimal ----"));
...
"#;
    ...
    fn emits_maude_model_for_minimal_workflow() {
        ...
        assert!(model.contains("mod WHIPPLETREE-MINIMAL is"));
        assert!(model.contains(
            "*** - declaredEffectsOnly: static validation plus generated DeclaredEffectType"
        ));
