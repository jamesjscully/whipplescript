...
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crate lives under <repo>/crates/whippletree-cli")
        .to_path_buf()
}
...
fn e2e_real_baml_http_coerce_when_enabled() {
    if env::var("WHIPPLETREE_RUN_BAML_E2E").ok().as_deref() != Some("1") {
        eprintln!("skipping real BAML e2e because WHIPPLETREE_RUN_BAML_E2E=1 is not set");
        return;
    }

    let baml_url =
        env::var("WHIPPLETREE_BAML_URL").expect("WHIPPLETREE_BAML_URL is required for real BAML e2e");
...
fn e2e_no_default_runtime_state_is_tracked_in_examples() {
    let default_store = fixture("examples/workflows/.whippletree/workflows/Minimal.sqlite");
