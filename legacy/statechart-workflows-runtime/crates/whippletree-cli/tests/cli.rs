...
fn validate_accepts_workflow() {
    ...
    let output = run_json(whippletree().arg("validate").arg(file).arg("--json"));
...
fn init_scaffolds_valid_local_project_and_refuses_overwrite() {
    ...
    let policy = dir.path().join(".whippletree/policy.json");
    let state_dir = dir.path().join(".whippletree/state");
    let workflow_store_dir = dir.path().join(".whippletree/workflows");
    ...
    let validation = run_json(whippletree().arg("validate").arg(&workflow).arg("--json"));
    assert_eq!(validation["ok"], true);

    let policy_validation = run_json(whippletree().arg("validate-policy").arg(&policy).arg("--json"));
    assert_eq!(policy_validation["ok"], true);

    let duplicate = whippletree()
        .arg("init")
        .arg(dir.path())
        .output()
        .expect("command runs");
...
fn emit_run_status_events_and_log_share_the_same_store() {
    ...
    let queued_events_text = whippletree()
        .arg("events")
        .arg(&file)
        .arg("--store")
        .arg(&store)
...
"#,
    ...
    let compact_status = whippletree()
        .arg("status")
        .arg(&file)
        .arg("--store")
        .arg(&store)
...
"#,
    ...
    let overview = run_json(whippletree().arg("overview").arg(&file).arg("--json"));
...
fn validate_policy_accepts_policy_without_workflow() {
    ...
    let output = run_json(whippletree().arg("validate-policy").arg(&policy).arg("--json"));
...
fn emit_model_outputs_tla_module() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("---- MODULE Whippletree_CliWorkflow ----"));
...
fn emit_model_outputs_maude_module() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("mod WHIPPLETREE-CLIWORKFLOW is"));
...
fn build_writes_ir_and_tla_artifacts() {
    ...
    assert!(output["artifact_hashes"]["baml_src/workflow.baml"]
        ...
        .expect("baml hash")
        .starts_with("sha256:"));
    assert!(tla_model.ends_with("Whippletree_CliWorkflow.tla"));
    assert!(tla_config.ends_with("Whippletree_CliWorkflow.cfg"));
    assert!(maude_model.ends_with("WHIPPLETREE-CLIWORKFLOW.maude"));
    ...
    assert!(std::fs::read_to_string(tla_model)
        .expect("tla reads")
        .contains("---- MODULE Whippletree_CliWorkflow ----"));
    ...
    assert!(std::fs::read_to_string(maude_model)
        .expect("maude reads")
        .contains("mod WHIPPLETREE-CLIWORKFLOW is"));
}
