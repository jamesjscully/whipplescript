//! Workflow language layer for `.whip` files.
//!
...
//! static validation. Runtime execution belongs in `whipplescript-engine`.
...
pub mod syntax {
    ...
    pub enum WhippleScriptLanguage {}

    impl Language for WhippleScriptLanguage {
    ...
    pub type SyntaxNode = rowan::SyntaxNode<WhippleScriptLanguage>;
    ...
    impl SyntaxBuilder {
        ...
        pub(crate) fn start_node(&mut self, kind: SyntaxKind) {
            self.builder.start_node(WhippleScriptLanguage::kind_to_raw(kind));
        }
        ...
        fn emit_through(&mut self, index: usize) {
            while self.emit_cursor <= index {
                ...
                self.builder.token(
                    WhippleScriptLanguage::kind_to_raw(token.kind),
                    token.text.as_str(),
                );
                self.emit_cursor += 1;
            }
...
pub mod source {
    ...
    pub enum SourceError {
        #[error("failed to parse WhippleScript source")]
...
"#,
    ...
    fn parse_with_file_records_workflow_source_path() {
        let source = include_str!("../../../examples/workflows/minimal.whip");
        let ir = crate::parse_source_with_file(source, "examples/workflows/minimal.whip")
            .expect("minimal source parses");

        assert_eq!(
            ir.workflow.source_path.as_deref(),
            Some("examples/workflows/minimal.whip")
        );
    }
