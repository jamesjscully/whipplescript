use whipplescript_engine::queue::{EventStatus, WorkflowEvent};
use whipplescript_engine::storage::WorkflowStore;
use whipplescript_engine::WorkflowRuntime;
...
#[derive(Debug, Parser)]
#[command(name = "whip")]
#[command(about = "WhippleScript statechart workflow CLI")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
impl From<CliModelTarget> for whipplescript_modelgen::ModelTarget {
...
enum CliError {
    ...
    #[error("{0}")]
    Source(#[from] whipplescript_workflow::SourceError),
    #[error("workflow validation failed")]
    Validation {
        diagnostics: Vec<whipplescript_workflow::Diagnostic>,
    },
    ...
    Storage(#[from] whipplescript_engine::storage::StorageError),
    #[error("runtime error: {0}")]
    Runtime(#[from] whipplescript_engine::RuntimeError),
    #[error("adapter bridge error: {0}")]
    AdapterBridge(#[from] whipplescript_engine::effects::EffectError),
    #[error("model generation error: {0}")]
    Modelgen(#[from] whipplescript_modelgen::ModelgenError),
...
#[derive(Debug, Serialize)]
struct RunOutput {
    outcome: Option<whipplescript_engine::EventProcessingOutcome>,
    status: whipplescript_engine::status::WorkflowStatus,
}
...
struct LogOutput {
    workflow_id: String,
    records: Vec<whipplescript_engine::log::WorkflowLogRecord>,
}
...
struct HarnessOnceOutput {
    workflow_id: String,
    invocation: Option<whipplescript_engine::storage::AgentInvocationRecord>,
...
struct HarnessStatusOutput {
    workflow_id: String,
    workflow_status: Option<whipplescript_engine::status::WorkflowStatus>,
    invocations: Vec<whipplescript_engine::storage::AgentInvocationRecord>,
    completions: Vec<whipplescript_engine::storage::AgentCompletionRecord>,
    harness_events: Vec<whipplescript_engine::storage::HarnessEventRecord>,
    recent_failures: Vec<whipplescript_engine::storage::HarnessEventRecord>,
}
...
struct OverviewOutput {
    ...
    status: Option<whipplescript_engine::status::WorkflowStatus>,
}
...
fn run(cli: Cli) -> Result<(), CliError> {
    match cli.command {
        ...
        Command::Validate {
            ...
            let parsed =
                whipplescript_workflow::parse_syntax_with_file(&source, file.display().to_string());
            let mut diagnostics = parsed.diagnostics;
            if let Some(ir) = parsed.ir {
                diagnostics.extend(whipplescript_workflow::validate_ir(&ir).diagnostics);
                    ...
                    || agent_file.is_some()
                {
                    ...
                    diagnostics.extend(whipplescript_adapters::validate_adapter_manifests(&manifests));
                    diagnostics.extend(whipplescript_adapters::validate_workflow_effects(
                        &ir, &manifests,
                    ));
                    diagnostics.extend(whipplescript_adapters::validate_workflow_policy(
                        &ir, &manifests, &policies,
                    ));
                } else {
                    diagnostics.extend(whipplescript_adapters::validate_policy_documents(&policies));
                }
            }
        ...
        Command::ValidateAdapter { manifests, json } => {
            ...
            let diagnostics = whipplescript_adapters::validate_adapter_manifests(&parsed_manifests);
            let ok = !diagnostics
                .iter()
                .any(|diagnostic| diagnostic.severity == whipplescript_workflow::Severity::Error);
        ...
        Command::ValidatePolicy { policies, json } => {
            ...
            let diagnostics = whipplescript_adapters::validate_policy_documents(&parsed_policies);
            let ok = !diagnostics_have_errors(&diagnostics);
        ...
        Command::Emit {
            ...
            if event.event_type == "humanReview.responded" {
                if let Some(review_file) = review_file.as_deref() {
                    whipplescript_adapters::record_human_review_response(review_file, &event.payload)?;
                }
            }
            if event.event_type == "finished" {
                if let Some(agent_file) = agent_file.as_deref() {
                    whipplescript_adapters::record_agent_finished_event(agent_file, &event.payload)?;
                }
            }
        ...
        Command::Run {
            ...
            if fake_coerce_outputs.is_empty() {
                if let Some(url) = &baml_url {
                    let diagnostics = whipplescript_adapters::validate_baml_http_policy(&policies, url);
            ...
            let store_baml_raw_response =
                whipplescript_adapters::should_store_baml_raw_response(&policies);
            let workflow_id = ir.workflow.name.clone();
            let store_path = store.unwrap_or_else(|| default_store_path(&file, &workflow_id));
            ensure_parent_dir(&store_path)?;
            let store = WorkflowStore::open(&store_path)?;
        ...
        Command::RetryEvent {
            ...
            if !store_path.exists() {
                return Err(whipplescript_engine::storage::StorageError::EventNotFound {
                    workflow_id,
                    event_id,
                }
                .into());
        ...
        Command::EmitModel {
            ...
            println!("{}", whipplescript_modelgen::emit_model(&ir, target.into())?);
            Ok(())
        }
        Command::EmitConfig {
            file,
...
fn emit_check_config(target: CliModelTarget) -> Result<String, CliError> {
    Ok(whipplescript_modelgen::emit_check_config(target.into())?)
}

fn load_ir(file: &PathBuf) -> Result<whipplescript_workflow::WorkflowIr, CliError> {
    let source = load_source(file)?;
    Ok(whipplescript_workflow::parse_source_with_file(
        &source,
        file.display().to_string(),
    )?)
}

fn load_valid_ir(file: &PathBuf) -> Result<whipplescript_workflow::WorkflowIr, CliError> {
    let ir = load_ir(file)?;
...
fn load_valid_ir_with_contract_shortcuts(
    ...
    (
        whipplescript_workflow::WorkflowIr,
        Vec<whipplescript_adapters::AdapterManifest>,
        Vec<whipplescript_adapters::CapabilityPolicyDocument>,
    ),
    CliError,
> {
    let mut manifests = load_valid_adapter_manifests(adapter_manifests)?;
...
fn load_valid_ir_with_contracts(
    file: &PathBuf,
    manifests: &[whipplescript_adapters::AdapterManifest],
    policies: &[whipplescript_adapters::CapabilityPolicyDocument],
) -> Result<whipplescript_workflow::WorkflowIr, CliError> {
    let ir = load_valid_ir(file)?;
...
fn validate_ir_contracts(
    ir: &whipplescript_workflow::WorkflowIr,
    manifests: &[whipplescript_adapters::AdapterManifest],
    policies: &[whipplescript_adapters::CapabilityPolicyDocument],
) -> Result<(), CliError> {
    let mut diagnostics = Vec::new();
    if !manifests.is_empty() {
        diagnostics.extend(whipplescript_adapters::validate_workflow_effects(ir, manifests));
    }
    if !policies.is_empty() {
        diagnostics.extend(whipplescript_adapters::validate_workflow_policy(
            ir, manifests, policies,
...
fn load_status_for_file(
    ...
    include_review_manifest: bool,
    include_agent_manifest: bool,
) -> Result<whipplescript_engine::status::WorkflowStatus, CliError> {
    let mut manifests = load_valid_adapter_manifests(adapter_manifests)?;
    add_file_backed_manifests_if_needed(
        &mut manifests,
        include_plan_manifest,
    ...
    if store_path.exists() {
        ...
        let data = whipplescript_engine::initial_context_from_ir(&ir);
        Ok(whipplescript_engine::status::WorkflowStatus {
            workflow_id: workflow_id.clone(),
            workflow_name: workflow_id,
            current_state: whipplescript_engine::initial_state_name(&ir),
            blocked_reason: None,
            data_summary: whipplescript_engine::summarize_status_data(&data),
            data,
            pending_events: 0,
...
fn load_overview_for_file(
    ...
    let source = load_source(file)?;
    let parsed = whipplescript_workflow::parse_syntax_with_file(&source, file.display().to_string());
    let mut diagnostics = parsed.diagnostics;
    let status = if let Some(ir) = parsed.ir {
        ...
        Some(if store_path.exists() {
            ...
            let data = whipplescript_engine::initial_context_from_ir(&ir);
            whipplescript_engine::status::WorkflowStatus {
                workflow_id: workflow_id.clone(),
                workflow_name: workflow_id,
                current_state: whipplescript_engine::initial_state_name(&ir),
                blocked_reason: None,
...
fn init_project(dir: &Path, name: &str, force: bool) -> Result<InitOutput, CliError> {
    ...
    let whipplescript_dir = dir.join(".whipplescript");
    let state_dir = whipplescript_dir.join("state");
    fs::create_dir_all(&state_dir).map_err(|source| CliError::CreateDir {
        path: state_dir.clone(),
        source,
    })?;
    let workflow_store_dir = whipplescript_dir.join("workflows");
    ...
    let policy = whipplescript_dir.join("policy.json");
...
fn load_valid_adapter_manifests(
    manifests: &[PathBuf],
) -> Result<Vec<whipplescript_adapters::AdapterManifest>, CliError> {
    let parsed_manifests = load_adapter_manifests(manifests)?;
    let diagnostics = whipplescript_adapters::validate_adapter_manifests(&parsed_manifests);
    if diagnostics_have_errors(&diagnostics) {
...
fn load_valid_policy_documents(
    policies: &[PathBuf],
) -> Result<Vec<whipplescript_adapters::CapabilityPolicyDocument>, CliError> {
    let parsed_policies = load_policy_documents(policies)?;
    let diagnostics = whipplescript_adapters::validate_policy_documents(&parsed_policies);
    if diagnostics_have_errors(&diagnostics) {
        return Err(policy_document_validation_error(diagnostics));
...
fn adapter_dispatcher_from_manifests(
    manifests: Vec<whipplescript_adapters::AdapterManifest>,
    policies: Vec<whipplescript_adapters::CapabilityPolicyDocument>,
    plan_file: Option<PathBuf>,
    review_file: Option<PathBuf>,
    agent_file: Option<PathBuf>,
) -> Box<dyn whipplescript_engine::effects::EffectDispatcher> {
    if manifests.is_empty() {
        Box::new(whipplescript_engine::effects::NoopEffectDispatcher)
    } else {
        let dispatcher = whipplescript_adapters::ManifestEffectDispatcher::from_manifests(manifests)
...
fn add_json_plan_manifest_if_needed(manifests: &mut Vec<whipplescript_adapters::AdapterManifest>) {
    ...
    if !has_plan_manifest {
        manifests.push(whipplescript_adapters::json_plan_adapter_manifest());
    }
}

fn add_file_backed_manifests_if_needed(
    manifests: &mut Vec<whipplescript_adapters::AdapterManifest>,
    include_plan_manifest: bool,
    include_review_manifest: bool,
    include_agent_manifest: bool,
) {
...
fn add_json_human_review_manifest_if_needed(
    ...
    if !has_review_manifest {
        manifests.push(whipplescript_adapters::json_human_review_adapter_manifest());
    }
}
...
fn add_json_human_review_response_event_manifest_if_needed(
    ...
    if !has_review_response_event {
        manifests.push(whipplescript_adapters::json_human_review_response_event_manifest());
    }
}

fn add_json_agent_manifest_if_needed(manifests: &mut Vec<whipplescript_adapters::AdapterManifest>) {
    ...
    if !has_agent_manifest {
        manifests.push(whipplescript_adapters::json_agent_adapter_manifest());
    }
}
...
fn add_json_agent_finished_event_manifest_if_needed(
    ...
    if !has_agent_finished_event {
        manifests.push(whipplescript_adapters::json_agent_finished_event_manifest());
    }
}
...
) -> Box<dyn whipplescript_engine::coerce::CoerceExecutor> {
    if !fake_coerce_outputs.is_empty() {
        Box::new(whipplescript_engine::coerce::FakeCoerceExecutor::new(
            fake_coerce_outputs,
        ))
    } else if let Some(url) = baml_url {
        Box::new(
            whipplescript_engine::coerce::BamlHttpCoerceExecutor::new(url)
                .with_timeout_ms(baml_timeout_ms)
                .with_baml_src_hash(baml_source_hash(ir))
                .with_store_raw_response(store_baml_raw_response),
        )
    } else {
        Box::new(whipplescript_engine::coerce::NoopCoerceExecutor)
    }
}

fn baml_source_hash(ir: &whipplescript_workflow::WorkflowIr) -> Option<String> {
...
fn adapter_manifest_validation_error(diagnostics: Vec<whipplescript_workflow::Diagnostic>) -> CliError {
...
fn policy_document_validation_error(diagnostics: Vec<whipplescript_workflow::Diagnostic>) -> CliError {
    let message = diagnostics
        .iter()
        .map(|diagnostic| diagnostic.message.as_str())
        .collect::<Vec<_>>()
...
fn harness_profile_policy_validation_error(
    ...
    let message = diagnostics
        .iter()
        .filter(|diagnostic| diagnostic.severity == whipplescript_workflow::Severity::Error)
        .map(|diagnostic| diagnostic.message.as_str())
        .collect::<Vec<_>>()
        .join("; ");
...
fn workflow_contract_validation_error(diagnostics: Vec<whipplescript_workflow::Diagnostic>) -> CliError {
    let message = diagnostics
        .iter()
        .filter(|diagnostic| diagnostic.severity == whipplescript_workflow::Severity::Error)
        .map(|diagnostic| diagnostic.message.as_str())
...
fn load_adapter_manifests(
    ...
    for path in manifests {
        ...
        let manifest = serde_json::from_str::<whipplescript_adapters::AdapterManifest>(&source)
...
fn load_policy_documents(
    ...
    for path in policies {
        let source = load_source(path)?;
        let policy = serde_json::from_str::<whipplescript_adapters::CapabilityPolicyDocument>(&source)
...
fn diagnostics_have_errors(diagnostics: &[whipplescript_workflow::Diagnostic]) -> bool {
    diagnostics
        .iter()
        .any(|diagnostic| diagnostic.severity == whipplescript_workflow::Severity::Error)
}
...
fn print_diagnostics(diagnostics: &[whipplescript_workflow::Diagnostic]) {
    for diagnostic in diagnostics {
        eprintln!("{}", format_diagnostic(diagnostic));
    }
}

fn format_diagnostic(diagnostic: &whipplescript_workflow::Diagnostic) -> String {
    let severity = match diagnostic.severity {
        whipplescript_workflow::Severity::Error => "error",
        whipplescript_workflow::Severity::Warning => "warning",
        whipplescript_workflow::Severity::Note => "note",
    };
...
fn print_status(status: &whipplescript_engine::status::WorkflowStatus) {
    println!("workflow: {}", status.workflow_name);
    println!("state: {}", status.current_state);
...
fn print_compact_status(status: &whipplescript_engine::status::WorkflowStatus) {
    println!("workflow: {}", status.workflow_name);
    println!("state: {}", status.current_state);
    println!("waiting: {}", status_waiting_reason(status));
    println!("pending events: {}", status.pending_events);
...
fn status_waiting_reason(status: &whipplescript_engine::status::WorkflowStatus) -> String {
...
fn process_workflow_once(
    ...
    let ir = load_valid_ir_with_contracts(file, &manifests, &policies)?;
    let workflow_id = ir.workflow.name.clone();
    let store_baml_raw_response = whipplescript_adapters::should_store_baml_raw_response(&policies);
...
fn run_harness_once(
    ...
    let worker_id = format!("whipplescript-cli-{}", std::process::id());
    ...
    store.mark_agent_invocation_started(
        whipplescript_engine::storage::AgentInvocationStartedUpdate {
    ...
    let invocation_status = if output.timed_out {
        whipplescript_engine::storage::AgentInvocationStatus::TimedOut
    } else if succeeded {
        whipplescript_engine::storage::AgentInvocationStatus::Succeeded
    } else {
        whipplescript_engine::storage::AgentInvocationStatus::Failed
    };
    ...
    let mut completion_event = match build_cli_event(
        ...
        Err(error) => {
            store.mark_agent_invocation_exited(
                &claimed.invocation_id,
                whipplescript_engine::storage::AgentInvocationStatus::CompletionRejected,
                exit_code,
    ...
    completion_event.source = Some(whipplescript_engine::queue::EventSource {
        kind: "harness".to_string(),
        name: Some(agent_config.provider.clone()),
    });
    let completion = whipplescript_engine::storage::AgentCompletionRecord {
        workflow_id: workflow_id.clone(),
...
fn recover_expired_harness_leases(
    store: &WorkflowStore,
    workflow_id: &str,
) -> Result<Vec<whipplescript_engine::storage::AgentInvocationRecord>, CliError> {
...
) -> Vec<whipplescript_workflow::Diagnostic> {
    let mut diagnostics = Vec::new();
...
fn validate_harness_provider_name(
    diagnostics: &mut Vec<whipplescript_workflow::Diagnostic>,
    profile: &str,
    provider: &str,
) {
...
fn profile_policy_error(message: String) -> whipplescript_workflow::Diagnostic {
    whipplescript_workflow::Diagnostic {
        severity: whipplescript_workflow::Severity::Error,
        message,
        span: None,
    }
}
...
fn resolve_harness_provider_command(
    config: &HarnessAgentConfig,
    invocation: &whipplescript_engine::storage::AgentInvocationRecord,
...
fn run_harness_command(
    ...
    command.env("WHIPPLESCRIPT_WORKFLOW_ID", workflow_id);
    command.env("WHIPPLESCRIPT_INVOCATION_ID", &invocation.invocation_id);
    command.env("WHIPPLESCRIPT_AGENT", &invocation.agent);
    command.env(
        "WHIPPLESCRIPT_INPUT_JSON",
        serde_json::to_string(&invocation.input)?,
    );
    command.env("WHIPPLESCRIPT_RUN_DIR", run_dir);
    if let Some(message) = invocation
        .input
        .get("message")
        .and_then(serde_json::Value::as_str)
    {
        command.env("WHIPPLESCRIPT_PROMPT", message);
    } else {
        command.env("WHIPPLESCRIPT_PROMPT", serde_json::to_string(&invocation.input)?);
    }
...
fn append_harness_event(
    ...
    store.append_harness_event(&whipplescript_engine::storage::HarnessEventRecord {
...
fn stored_status(
    ...
    Ok(whipplescript_engine::project_status(ir, store, workflow_id)?)
}
...
fn check_workflow(
    ...
    match target {
        ...
        CliModelTarget::Apalache | CliModelTarget::Veil => {
            let _ = whipplescript_modelgen::emit_model(ir, target.into())?;
            unreachable!("unsupported targets currently return modelgen errors")
        }
    }
}

fn prove_workflow(ir: &whipplescript_workflow::WorkflowIr) -> Result<ProveOutput, CliError> {
    let checks = vec![
...
fn check_tla_workflow(ir: &whipplescript_workflow::WorkflowIr) -> Result<CheckOutput, CliError> {
    let model = whipplescript_modelgen::emit_model(ir, whipplescript_modelgen::ModelTarget::Tla)?;
    let module_name = tla_module_name(&model).unwrap_or_else(|| "WhippleScriptModel".to_string());
    ...
    fs::write(&config_path, whipplescript_modelgen::emit_tla_check_config()).map_err(|source| {
        CliError::Write {
...
fn check_maude_workflow(ir: &whipplescript_workflow::WorkflowIr) -> Result<CheckOutput, CliError> {
    let model = whipplescript_modelgen::emit_model(ir, whipplescript_modelgen::ModelTarget::Maude)?;
    let module_name = maude_module_name(&model).unwrap_or_else(|| "WHIPPLESCRIPT-MODEL".to_string());
    let base = std::env::temp_dir().join(format!("{module_name}_{}", ulid::Ulid::new()));
    fs::create_dir_all(&base).map_err(|source| CliError::CreateDir {
        path: base.clone(),
        source,
...
fn build_workflow(
    ...
    let tla_source = whipplescript_modelgen::emit_model(ir, whipplescript_modelgen::ModelTarget::Tla)?;
    ...
    let maude_source = whipplescript_modelgen::emit_model(ir, whipplescript_modelgen::ModelTarget::Maude)?;
    ...
    fs::write(&tla_config, whipplescript_modelgen::emit_tla_check_config()).map_err(|source| {
        CliError::Write {
...
fn emit_baml_source(ir: &whipplescript_workflow::WorkflowIr) -> String {
    ...
    for (name, schema) in &ir.types {
        ...
        match schema {
            whipplescript_workflow::schema::Schema::Enum { values } => {
            ...
            whipplescript_workflow::schema::Schema::Record { fields } => {
    ...
    for function in ir.coerce_functions.values() {
        if let Some(model) = &function.model {
            ...
            model_clients
                .entry(model.clone())
                .or_insert_with(|| format!("WhippleScriptLLM{next_index}"));
        }
    }
...
fn baml_reachable_types(ir: &whipplescript_workflow::WorkflowIr) -> BTreeSet<String> {
...
fn collect_baml_type_refs(
    schema: &whipplescript_workflow::schema::Schema,
    ir: &whipplescript_workflow::WorkflowIr,
    names: &mut BTreeSet<String>,
) {
    match schema {
        whipplescript_workflow::schema::Schema::Ref { name } => {
        ...
        whipplescript_workflow::schema::Schema::Optional { inner }
        | whipplescript_workflow::schema::Schema::List { inner }
        | whipplescript_workflow::schema::Schema::Set { inner } => {
            collect_baml_type_refs(inner, ir, names);
        }
        whipplescript_workflow::schema::Schema::Map { key, value } => {
            collect_baml_type_refs(key, ir, names);
            collect_baml_type_refs(value, ir, names);
        }
        whipplescript_workflow::schema::Schema::Union { variants } => {
        ...
        whipplescript_workflow::schema::Schema::String
        | whipplescript_workflow::schema::Schema::Int
        | whipplescript_workflow::schema::Schema::Float
        | whipplescript_workflow::schema::Schema::Boolean
        | whipplescript_workflow::schema::Schema::Null
        | whipplescript_workflow::schema::Schema::Time
        | whipplescript_workflow::schema::Schema::Duration
        | whipplescript_workflow::schema::Schema::Agent
        | whipplescript_workflow::schema::Schema::Literal { .. }
        | whipplescript_workflow::schema::Schema::Enum { .. }
        | whipplescript_workflow::schema::Schema::Json => {}
    }
}

fn baml_type(schema: &whipplescript_workflow::schema::Schema) -> String {
    match schema {
        whipplescript_workflow::schema::Schema::String
        | whipplescript_workflow::schema::Schema::Time
        | whipplescript_workflow::schema::Schema::Duration
        | whipplescript_workflow::schema::Schema::Agent => "string".to_string(),
        whipplescript_workflow::schema::Schema::Int => "int".to_string(),
        whipplescript_workflow::schema::Schema::Float => "float".to_string(),
        whipplescript_workflow::schema::Schema::Boolean => "bool".to_string(),
        whipplescript_workflow::schema::Schema::Null => "null".to_string(),
        whipplescript_workflow::schema::Schema::Literal { value } => match value {
        ...
        whipplescript_workflow::schema::Schema::Enum { values } => values.join(" | "),
        whipplescript_workflow::schema::Schema::Optional { inner } => format!("{}?", baml_type(inner)),
        whipplescript_workflow::schema::Schema::List { inner }
        | whipplescript_workflow::schema::Schema::Set { inner } => format!("{}[]", baml_type(inner)),
        whipplescript_workflow::schema::Schema::Map { key, value } => {
            format!("map<{}, {}>", baml_type(key), baml_type(value))
        }
        whipplescript_workflow::schema::Schema::Union { variants } => variants
            .iter()
            .map(baml_type)
            .collect::<Vec<_>>()
            .join(" | "),
        whipplescript_workflow::schema::Schema::Record { .. }
        | whipplescript_workflow::schema::Schema::Json => "string".to_string(),
        whipplescript_workflow::schema::Schema::Ref { name } => name.clone(),
    }
}
