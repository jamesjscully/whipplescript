---
name: whippletree-statechart
description: Use when coding agents need to author, validate, run, inspect, or repair Whippletree `.whip` statechart workflows for orchestrating agents through durable events, typed effects, adapter manifests, and formal checks.
---

# Whippletree Statechart Workflows

Use Whippletree as a restricted workflow runtime for coordinating agents. The
...
- Whippletree owns durable workflow mechanics: parsing, validation, event queue,
...
1. Confirm the new CLI surface is available: `whip --help`. From source,
   build with `cargo build -p whippletree-cli` and use `target/debug/whip`.
2. Find the workflow file, usually `examples/workflows/*.whip` or a repo
   workflow under `.whippletree/workflows/`.
...
When an effect argument expression has a known schema, Whippletree uses it inside
...
When an adapter manifest or policy is supplied, Whippletree validates
