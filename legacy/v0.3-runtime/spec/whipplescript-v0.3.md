# WhippleScript v0.3 - Normative Specification

## 1. Definition

**WhippleScript** is a lightweight local daemon and CLI for running **ordinary user-authored programs** in response to schedules, file changes, emitted events, and long-running process sources.

WhippleScript exists to support reactive development and multi-agent workflows without becoming a workflow engine.

The core promise is:

**Write normal scripts. WhippleScript keeps them wired to the world.**

WhippleScript is best understood as:
...
WhippleScript invokes user-authored orchestrators. It does not become the orchestrator.

---

## 2. Design Center

WhippleScript assumes that the user may be a **competent coding agent** or a technically sophisticated human. Therefore, WhippleScript should not hide meaningful control logic behind protective abstractions.

WhippleScript should abstract only the machinery that every reliable reactive script runner would otherwise have to reimplement:
...
WhippleScript should leave all domain-relevant behavior in user space:
...
A conforming WhippleScript implementation **MUST** satisfy all **MUST** requirements.

---

## 4. Core Non-Goals

WhippleScript **MUST NOT** require users to model work as:
...
WhippleScript **MUST NOT** define privileged daemon-level meanings for:
...
WhippleScript **MAY** provide recipes for common patterns, but recipes **MUST** expand into editable config and editable user scripts.

WhippleScript **MUST NOT** require an SDK. Every core feature **SHOULD** be usable from shell scripts.

---

## 5. Core Objects

WhippleScript has eight core objects:
...
A **Task** is a finite command run by WhippleScript.
...
A **Service** is a long-running command managed by WhippleScript.
...
WhippleScript **SHOULD** support these trigger classes:
...
For implementation consistency, primitive triggers **SHOULD** be normalized internally as WhippleScript events before task admission is evaluated.
...
A **Source** is a process or built-in mechanism that emits WhippleScript events.
...
Built-in sources such as schedules and file watchers **SHOULD** emit mechanical WhippleScript events into the same event log used by user-authored sources.

WhippleScript **MUST NOT** emit both a hidden direct trigger and a separate routable event for the same occurrence in a way that starts the same task twice.
...
WhippleScript events are generic. Event type strings are application space.

A conforming event envelope **MUST** contain:

```ts
type WhippleScriptEvent = {
  id: string
  type: string
  time: string
  source: string
...
```ts
type OptionalWhippleScriptEventFields = {
  workspace?: string
...
Older SDKs or user payloads may use camelCase fields, but WhippleScript-owned event
envelope fields **SHOULD** use the snake_case names above for consistency with
run and trigger records.

WhippleScript **MUST NOT** assign domain semantics to event types.
...
WhippleScript v0.3 uses narrow local event delivery semantics.
...
WhippleScript v0.3 **MUST NOT** automatically replay historical events to newly added or changed tasks.

WhippleScript v0.3 **MUST NOT** provide durable promise, subscription cursor, distributed queue, or exactly-once delivery semantics.
...
Logs remain per run. WhippleScript **SHOULD NOT** merge logs from multiple restart attempts into one inseparable log stream.
...
WhippleScript **MUST** keep stdout and stderr inspectable per run.
...
A **Runtime** is the daemon's current mechanical view of configured and observed WhippleScript-managed processes for a workspace.
...
WhippleScript **SHOULD** expose runtime state through CLI and JSON interfaces.
...
A typical WhippleScript project contains:
...
The `.whipplescript/` directory contains user-inspectable project config and artifacts.

Internal daemon stores, sockets, lock files, and indexes **MAY** live outside the repository checkout. If an implementation stores internal state outside `.whipplescript/`, it **SHOULD** expose the state location through `whip doctor` or JSON status.
...
WhippleScript **MUST NOT** embed user TypeScript into the daemon process as the default execution model.

WhippleScript **SHOULD** run user code out-of-process.
...
emit WhippleScript events
write project state
construct traces
judge success/failure
```

WhippleScript **MUST NOT** require these decisions to be expressed in WhippleScript config.
...
const event = JSON.parse(process.env.WHIPPLESCRIPT_EVENT_JSON!)
...
Incorrect as WhippleScript core:
...
WhippleScript launches the script. The script owns the orchestration.

---

## 17. Configuration

WhippleScript **SHOULD** use a human-editable config format. TOML is recommended.
...
When WhippleScript starts a task or service, it **MUST** provide mechanical context.
...
WHIPPLESCRIPT_NAME
WHIPPLESCRIPT_KIND          # task | service
WHIPPLESCRIPT_WORKSPACE
WHIPPLESCRIPT_WORKSPACE_ROOT
WHIPPLESCRIPT_CONFIG_DIR
WHIPPLESCRIPT_STATE_DIR
WHIPPLESCRIPT_RUN_DIR
WHIPPLESCRIPT_EVENT_JSON
WHIPPLESCRIPT_EVENT_PATH
WHIPPLESCRIPT_EVENT_PAYLOAD_JSON
WHIPPLESCRIPT_PAYLOAD_JSON
WHIPPLESCRIPT_CONFIG_VERSION
WHIPPLESCRIPT_CORRELATION_ID
```

For small events, `WHIPPLESCRIPT_EVENT_JSON` is acceptable.

For large events, WhippleScript **SHOULD** write the event to a file and provide `WHIPPLESCRIPT_EVENT_PATH`.

Each run **MUST** have a private run directory, for example:

```text
.whipplescript/runs/run_01HV.../
  event.json
  meta.json
  stdout.log
  stderr.log
...
Scripts **SHOULD** use `WHIPPLESCRIPT_RUN_DIR` for temporary run-local files.

WhippleScript **MUST** allow scripts to ignore all WhippleScript-specific environment variables.
...
WhippleScript **MUST** support concurrent task and service runs as independent OS processes.

If multiple triggers fire at once, WhippleScript **MAY** run multiple scripts concurrently, subject only to explicit admission/resource policy.

WhippleScript **MUST** record and expose each run independently.

WhippleScript **MUST** isolate run metadata and logs.

WhippleScript **MUST NOT** impose a workflow-level concurrency model across runs.

Semantic coordination among concurrent scripts belongs in user code.

WhippleScript-owned concurrency mechanics include:
...
**WhippleScript handles concurrent processes. User code handles concurrent meanings.**
...
WhippleScript **MUST NOT** silently drop triggers. If a trigger is rejected, coalesced, or superseded by admission policy, that fact **MUST** be inspectable.
...
WhippleScript **MAY** expose BEAM-inspired supervision primitives for configured processes.
...
If a process exceeds its restart budget, WhippleScript **SHOULD** mark it as failed and stop restarting it until user intervention or config reload.

### 21.4 Supervisor Strategy

WhippleScript v0.3 **SHOULD** support only one-for-one supervision:

```text
if process X fails, restart process X according to X's policy
```

WhippleScript **SHOULD NOT** implement BEAM-style group strategies initially:
...
WhippleScript **MAY** support health checks for services.
...
WhippleScript **MUST NOT** interpret domain-specific health from logs or payloads unless the user health-check script encodes that interpretation.

---

## 23. Resource Policy

WhippleScript **MAY** enforce hard process resource limits.
...
WhippleScript **MAY** provide named locks.
...
WhippleScript does not know what `branch:main` means. It only provides atomic exclusion.

WhippleScript **MUST NOT** implicitly lock based on task names, event types, files, branches, agents, or payload fields.
...
An implementation **MAY** allow tokenless release only when the caller is mechanically identifiable as the owning WhippleScript run.

If a lock is acquired by a run and that run exits, WhippleScript **SHOULD** release the lock automatically or allow the lease to expire.

If a lock is acquired outside a run, WhippleScript **SHOULD** require an explicit lease duration and make the owner inspectable.

WhippleScript **SHOULD** provide an explicit administrative force-release operation that requires a reason and records the previous owner/token for audit.

WhippleScript **SHOULD** provide an ergonomic `lock with` command or SDK helper that acquires a lock, runs a command or callback, and releases with the returned token.

WhippleScript **SHOULD** expose held locks through a CLI or status interface.

WhippleScript **MUST NOT** infer stale locks from domain concepts. Staleness, if implemented, is based only on mechanical facts such as owner process death or lease expiration.

---

## 25. Event Emission

WhippleScript **SHOULD** allow scripts to emit events.
...
`emit` is the canonical event creation verb. WhippleScript **SHOULD NOT** add a
...
```ts
import { emit } from "@whipplescript/sdk"

await emit("review.ready", { branch: "feature-x" })
```

When an event is emitted from inside a run, WhippleScript **SHOULD** attach mechanical lineage:
...
WhippleScript **SHOULD** support optional event causality fields:
...
```text
source_run_id  the WhippleScript run that emitted this event, if known
parent_event_id the WhippleScript event visible to that run, if known
correlation_id a shared id for a broader episode, if known
```

WhippleScript may propagate these mechanically.

WhippleScript **MUST NOT** infer high-level traces, workflows, spans, or plans from these fields.
...
WhippleScript **SHOULD** automatically reconcile declared service state with observed runtime state.
...
WhippleScript **SHOULD** reconcile automatically after:
...
WhippleScript v0.3 **SHOULD** model service reconciliation with a small desired-state contract.
...
WhippleScript **MUST** expose runtime status.
...
const status = await whipplescript.status()
...
for (const service of failedSources) {
  await whipplescript.emit("runtime.service_failed", {
...
WhippleScript **SHOULD** support explicit project runtime lifecycle commands.
...
WhippleScript command names **SHOULD** keep project lifecycle separate from service lifecycle.
...
WhippleScript **MAY** expose a dry-run reconciliation command:
...
what mechanical reconciliation actions would WhippleScript take?
```
...
WhippleScript **MUST** maintain internal state sufficient to inspect events, runs, logs, services, and runtime status.

WhippleScript **SHOULD NOT** make an application key-value store part of the core conceptual model.

Instead, WhippleScript **SHOULD** expose useful paths:
...
WhippleScript **MAY** offer a convenience scratchpad later, but it **MUST NOT** become the recommended application-state abstraction.

---

## 32. Schema Validation

WhippleScript **MUST** validate its own event envelope and config.

WhippleScript **SHOULD NOT** validate domain payload schemas in the daemon core.
...
WhippleScript **SHOULD** support file-settling behavior because editors often produce bursts of writes.
...
WhippleScript **SHOULD NOT** generalize this into broad event-stream semantic debounce or dedupe.
...
WhippleScript should treat integrations suspiciously. Adapters are where domain semantics often sneak into the daemon.
...
WhippleScript **MAY** ship shallow built-in adapters, but built-in adapters **MUST** only observe, translate, and emit events.
...
WhippleScript **SHOULD** provide recipes as scaffolding.
...
WhippleScript **SHOULD** provide a TypeScript SDK as the golden-path ergonomic layer.
...
**Could this helper be explained as a thin wrapper around environment variables, subprocesses, and the WhippleScript CLI?**
...
WhippleScript **MUST** provide a CLI.
...
The CLI should make WhippleScript feel inspectable, not magical.
...
WhippleScript **SHOULD** hot reload config.
...
WhippleScript **MUST** distinguish mechanical process failure from semantic failure.
...
WhippleScript owns the first category. User code owns the second.
...
WhippleScript **MUST NOT** inspect domain output to override this interpretation.

---

## 40. Security Model

WhippleScript executes user commands. Therefore, WhippleScript is a code execution tool.

WhippleScript **MUST NOT** pretend configured scripts are safe.

WhippleScript **SHOULD** provide strong inspection:
...
await $`tool-cli logs ${runId} --tail 200 > ${process.env.WHIPPLESCRIPT_RUN_DIR}/tool.log`

await $`review-cli request --run ${runId} --log ${process.env.WHIPPLESCRIPT_RUN_DIR}/tool.log`

await $`whip emit review.requested --json ${JSON.stringify({ runId })}`
```

WhippleScript does not know what a review is.
...
The user owns the fanout and join. WhippleScript only launched the script and recorded the run.
...
If this source crashes, WhippleScript may restart it. That is process supervision, not workflow orchestration.
...
```ts
import { withLock } from "@whipplescript/sdk"
import { $ } from "zx"
...
WhippleScript does not know what `branch:main` means.
...
```ts
import { whipplescript, emit } from "@whipplescript/sdk"
...
WhippleScript exposes runtime facts. The script decides what they mean.
...
Every proposed WhippleScript feature should pass these tests.
...
Could an advanced user replace WhippleScript with cron, watchexec, shell scripts, tmux, and systemd user services, but prefer WhippleScript because it is cleaner?

If yes, WhippleScript is in the right design space.

If not, it may be becoming a platform.

### 42.7 The Runtime-vs-Workflow Test

Is this state about declared and observed WhippleScript-managed processes?
...
WhippleScript core should contain only what is necessary to make user-authored reactive scripts reliable:
...
**WhippleScript is a small local daemon for reliably invoking, supervising, reconciling, and inspecting ordinary user programs in response to events, while leaving all orchestration semantics in normal user code.**
