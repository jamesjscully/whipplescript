...
impl Sandbox {
    fn new(config: &str) -> Self {
        ...
        fs::create_dir_all(root.path().join(".whipplescript")).expect("create config dir");
        fs::create_dir_all(root.path().join("scripts")).expect("create scripts dir");
        fs::write(root.path().join(".whipplescript/project.whip"), config.trim())
            .expect("write config");
    ...
    fn whipplescript<I, S>(&self, args: I) -> Output
    where
        ...
        S: AsRef<OsStr>,
    {
        Command::new(&self.bin)
            ...
            .output()
            .expect("run whipplescript")
    }

    fn whipplescript_with_stdin<I, S>(&self, args: I, stdin: &str) -> Output
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
    ...
    where
        ...
        S: AsRef<OsStr>,
    {
        let output = self.whipplescript_with_stdin(args, stdin);
        assert!(
...
fn event_service_lock_and_shutdown_flow_survives_real_cli_boundaries() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf %s "$WHIPPLESCRIPT_EVENT_TYPE" > event-type.txt
        printf %s "${{WHIPPLESCRIPT_CORRELATION_ID:-}}" > event-correlation.txt
        printf %s "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" > event-payload.json
        printf %s "$WHIPPLESCRIPT_PAYLOAD_JSON" > payload.json
        printf %s "$WHIPPLESCRIPT_WORKSPACE" > workspace.txt
        printf %s "$WHIPPLESCRIPT_WORKSPACE_ROOT" > workspace-root.txt
        printf %s "$WHIPPLESCRIPT_CONFIG_DIR" > config-dir.txt
        printf %s "$WHIPPLESCRIPT_STATE_DIR" > state-dir.txt
        printf %s "$WHIPPLESCRIPT_RUN_DIR" > run-dir.txt
    ...
    assert_eq!(
        ...
        sandbox.root().join(".whipplescript").display().to_string()
    );
...
fn lock_recovery_and_with_lock() {
    ...
    sandbox.ok([
        ...
        "sh",
        "-c",
        "printf '%s:%s' \"$WHIPPLESCRIPT_LOCK_NAME\" \"$WHIPPLESCRIPT_LOCK_TOKEN\" > with-lock.txt",
    ]);
...
fn object_cli_aliases() {
    ...
    sandbox.write_script(
        ...
        r#"
        #!/bin/sh
        printf "$WHIPPLESCRIPT_EVENT_TYPE" > event.txt
        "#,
    );
    sandbox.write_script(
        "worker.sh",
...
fn manual_run_cancel_logs_and_runs_are_coherent_from_the_cli() {
    ...
    sandbox.write_script(
        ...
        printf %s "$WHIPPLESCRIPT_RUN_ID" > active-run-id.txt
...
fn invalid_reload_preserves_running_daemon_and_valid_config_can_reload() {
    ...
    sandbox.write_file(".whipplescript/project.whip", good_config);
    sandbox.ok(["up"]);
    sandbox.ok(["down"]);
}
...
fn trigger_delivery_semantics_are_inspectable_from_the_cli() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf '%s %s\n' "$WHIPPLESCRIPT_EVENT_ID" "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" >> "$1"
        sleep "$2"
        "#,
    );
...
fn dynamic_service_lifecycle() {
    ...
    let original_config = sandbox.read(".whipplescript/project.whip");
    sandbox.write_script(
        "dynamic-service.sh",
        r#"
        #!/bin/sh
        set -eu
        printf '%s\n' "${MARKER:-missing}:${WHIPPLESCRIPT_NAME}:${WHIPPLESCRIPT_KIND}" >> dynamic-started.log
        trap "printf '%s\n' \"${MARKER:-missing}:${WHIPPLESCRIPT_NAME}\" >> dynamic-stopped.log; exit 0" TERM INT
        while true; do sleep 1; done
        "#,
    );
    ...
    assert_eq!(sandbox.read(".whipplescript/project.whip"), original_config);
...
fn dynamic_task_event_and_watch_lifecycle() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s|%s|%s\n' "$WHIPPLESCRIPT_NAME" "$WHIPPLESCRIPT_KIND" "${MARKER:-missing}" "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" >> dynamic-event.log
        "#,
    );
    sandbox.write_script(
        "dynamic-watch.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s\n' "$WHIPPLESCRIPT_NAME" "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" >> dynamic-watch.log
        "#,
    );
    sandbox.write_script(
        "dynamic-schedule.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s\n' "$WHIPPLESCRIPT_NAME" "$WHIPPLESCRIPT_EVENT_TYPE" >> dynamic-schedule.log
        "#,
    );
...
fn wait_and_subscribe_agent_flow() {
    ...
    sandbox.write_script(
        ...
        set -eu
        printf '%s' "$WHIPPLESCRIPT_RUN_ID" > active-run-id.txt
        printf '%s' "${WHIPPLESCRIPT_CORRELATION_ID:-}" > correlation.txt
        "#,
    );
...
fn adhoc_run_is_tracked_and_cancelable() {
    ...
    let started = sandbox.json([
        ...
        "sh",
        "-c",
        "printf '%s|%s|%s|%s' \"$PWD\" \"$EXTRA\" \"$WHIPPLESCRIPT_KIND\" \"$WHIPPLESCRIPT_PAYLOAD_JSON\"; printf 'err-line' >&2",
    ]);
...
#[test]
#[ignore = "opt-in stress coverage; run with `cargo test -p whipplescript-cli --test e2e -- --ignored sustained_stress_many_events_watch_changes_and_services`"]
...
fn run_sustained_load_scenario(
    ...
    sandbox.write_script(
        ...
        set -eu
        sleep 0.02
        printf '%s\n' "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" >> event-order.log
        "#,
    );
    sandbox.write_script(
        "record-watch.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s\n' "$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON" >> watch-events.log
        "#,
    );
    sandbox.write_script(
        "service.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s\n' "$WHIPPLESCRIPT_NAME:$WHIPPLESCRIPT_RUN_ID" >> services-started.log
        trap "exit 0" TERM INT
        while true; do sleep 1; done
        "#,
    );
