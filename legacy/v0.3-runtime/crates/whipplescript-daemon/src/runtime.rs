...
use whipplescript_core::{
    load_workspace_config, AdmissionConfig, AdmissionPolicy, WhippleScriptConfig, WhippleScriptError,
    WhippleScriptResult, EventId, EventRecord, EventRouting, ProcessState, ResourcePolicy, RestartMode,
...
pub struct DaemonHandle {
    socket_path: PathBuf,
    join_handle: Option<JoinHandle<WhippleScriptResult<()>>>,
}
...
impl DaemonServer {
    pub fn start(workspace: Workspace) -> WhippleScriptResult<DaemonHandle> {
    ...
    pub fn start_with_options(
    ...
    ) -> WhippleScriptResult<DaemonHandle> {
        ...
        let startup = (|| -> WhippleScriptResult<()> {
...
impl DaemonHandle {
    ...
    pub fn join(mut self) -> WhippleScriptResult<()> {
        if let Some(handle) = self.join_handle.take() {
            return handle
                .join()
                .map_err(|_| WhippleScriptError::internal("daemon thread panicked"))?;
        }
        Ok(())
    }
}
...
impl DaemonClient {
    ...
    pub fn inspect(&self) -> WhippleScriptResult<InspectResponse> {
        match self.send(DaemonRequest::Inspect)? {
            ResponsePayload::Inspect(response) => Ok(response),
            _ => Err(WhippleScriptError::internal("unexpected inspect response")),
        }
    }

    pub fn events(&self) -> WhippleScriptResult<Vec<EventRecord>> {
        match self.send(DaemonRequest::Events)? {
            ResponsePayload::Events { events } => Ok(events),
            _ => Err(WhippleScriptError::internal("unexpected events response")),
        }
    }

    pub fn triggers(&self) -> WhippleScriptResult<Vec<TriggerRecord>> {
        match self.send(DaemonRequest::Triggers)? {
            ResponsePayload::Triggers { triggers } => Ok(triggers),
            _ => Err(WhippleScriptError::internal("unexpected triggers response")),
        }
    }

    pub fn runs(&self) -> WhippleScriptResult<Vec<RunRecord>> {
        match self.send(DaemonRequest::Runs)? {
            ResponsePayload::Runs { runs } => Ok(runs),
            _ => Err(WhippleScriptError::internal("unexpected runs response")),
        }
    }

    pub fn start_task(&self, name: impl Into<String>) -> WhippleScriptResult<RunId> {
    ...
    pub fn start_task_with_provenance(
        ...
        match self.send(DaemonRequest::StartTask {
            ...
            _ => Err(WhippleScriptError::internal("unexpected task start response")),
        }
    }
    ...
    pub fn start_adhoc(
        ...
        match self.send(DaemonRequest::StartAdhoc {
            ...
            ResponsePayload::StartedRun { run_id } => Ok(run_id),
            _ => Err(WhippleScriptError::internal("unexpected ad hoc start response")),
        }
    }
    ...
    pub fn cancel_run(&self, run_id: impl Into<String>) -> WhippleScriptResult<()> {
    ...
    pub fn acquire_lock(
    ...
    ) -> WhippleScriptResult<ManualLockRecord> {
        ...
        match self.send(DaemonRequest::LockAcquire {
            ...
            _ => Err(WhippleScriptError::internal("unexpected lock acquire response")),
        }
    }
    ...
    pub fn renew_lock(
        ...
        match self.send(DaemonRequest::LockRenew {
            ...
            _ => Err(WhippleScriptError::internal("unexpected lock renew response")),
        }
    }
    ...
    pub fn force_release_lock(
        ...
        match self.send(DaemonRequest::LockForceRelease {
            ...
            _ => Err(WhippleScriptError::internal(
                "unexpected lock force-release response",
            )),
        }
    }

    pub fn locks(&self) -> WhippleScriptResult<Vec<ManualLockRecord>> {
        match self.send(DaemonRequest::LockStatus)? {
            ResponsePayload::Locks { locks } => Ok(locks),
            _ => Err(WhippleScriptError::internal("unexpected lock status response")),
        }
    }

    pub fn service_stop(&self, name: impl Into<String>) -> WhippleScriptResult<()> {
        self.expect_empty(DaemonRequest::ServiceStop { name: name.into() })
    }

    pub fn service_start(&self, name: impl Into<String>) -> WhippleScriptResult<()> {
        self.expect_empty(DaemonRequest::ServiceStart { name: name.into() })
    }

    pub fn service_restart(&self, name: impl Into<String>) -> WhippleScriptResult<()> {
    ...
    pub fn service_remove(&self, name: impl Into<String>) -> WhippleScriptResult<()> {
    ...
    pub fn task_remove(&self, name: impl Into<String>) -> WhippleScriptResult<()> {
        self.expect_empty(DaemonRequest::TaskRemove { name: name.into() })
    }

    pub fn reload_config(&self) -> WhippleScriptResult<()> {
        self.expect_empty(DaemonRequest::ReloadConfig)
    }

    pub fn shutdown(&self) -> WhippleScriptResult<()> {
        self.expect_empty(DaemonRequest::Shutdown)
    }

    fn expect_empty(&self, request: DaemonRequest) -> WhippleScriptResult<()> {
        match self.send(request)? {
            ResponsePayload::Empty => Ok(()),
            _ => Err(WhippleScriptError::internal("unexpected daemon response")),
        }
    }

    fn send(&self, request: DaemonRequest) -> WhippleScriptResult<ResponsePayload> {
        let mut stream = UnixStream::connect(&self.socket_path).map_err(|error| {
            WhippleScriptError::unavailable(format!(
        ...
        match response {
            ...
            DaemonResponse::Error { kind, message } => Err(WhippleScriptError {
                kind: kind.into(),
...
impl Runtime {
    ...
    fn run(&mut self) -> WhippleScriptResult<()> {
    ...
    fn accept_requests(&mut self) -> WhippleScriptResult<()> {
        loop {
            match self.listener.accept() {
                ...
                Err(error) => return Err(WhippleScriptError::internal(error.to_string())),
            }
        }
    }
    ...
    fn handle_request_inner(&mut self, request: DaemonRequest) -> WhippleScriptResult<ResponsePayload> {
        if self.shutdown_requested && !request_allowed_during_shutdown(&request) {
            return Err(WhippleScriptError::invalid_state("daemon is shutting down"));
        }
    ...
    fn start_task(
        ...
        route_result.started_run_id.ok_or_else(|| {
            WhippleScriptError::conflict(format!(
                "task {name:?} did not start immediately because admission policy queued or rejected it"
            ))
        })
    }

    fn start_adhoc(&mut self, request: AdhocRunRequest) -> WhippleScriptResult<RunId> {
    ...
    fn reload_config(&mut self) -> WhippleScriptResult<()> {
    ...
    fn apply_service_config_changes(&mut self, previous: &WhippleScriptConfig) -> WhippleScriptResult<()> {
    ...
    fn rebuild_runtime_state(&mut self) -> WhippleScriptResult<()> {
    ...
    fn add_dynamic_service(&mut self, request: DynamicServiceRequest) -> WhippleScriptResult<()> {
    ...
    fn remove_dynamic_service(&mut self, name: &str) -> WhippleScriptResult<()> {
        let active_run_id = {
    ...
    fn add_dynamic_task(&mut self, request: DynamicTaskRequest) -> WhippleScriptResult<()> {
    ...
    fn remove_dynamic_task(&mut self, name: &str) -> WhippleScriptResult<()> {
        let task = self
            .tasks
            .get(name)
            .ok_or_else(|| WhippleScriptError::not_found(format!("task {name:?} was not found")))?;
        if !task.dynamic {
            return Err(WhippleScriptError::conflict(format!(
    ...
    fn rebuild_schedules(&mut self) -> WhippleScriptResult<()> {
        let mut next = HashMap::new();
    ...
    fn rebuild_watches(&mut self) -> WhippleScriptResult<()> {
        let mut next = HashMap::new();
        for task in self.tasks.values() {
    ...
    fn reconcile_services(&mut self) -> WhippleScriptResult<()> {
        let now = Instant::now();
    ...
    fn poll_health_checks(&mut self) -> WhippleScriptResult<()> {
        let now = Instant::now();
        let service_names = self.services.keys().cloned().collect::<Vec<_>>();
        for name in service_names {
            let should_check = {
    ...
    fn poll_schedules(&mut self) -> WhippleScriptResult<()> {
        let now = Utc::now();
    ...
    fn poll_watches(&mut self) -> WhippleScriptResult<()> {
        ...
        for task_name in task_names {
            ...
            let watch = self
                .watches
                .get_mut(&task_name)
                .ok_or_else(|| WhippleScriptError::internal("watch state disappeared"))?;
    ...
    fn route_event(
        ...
        event: EventRecord,
        target: RouteTarget,
    ) -> WhippleScriptResult<RouteResult> {
    ...
    fn apply_admission(
        ...
        task_name: &str,
        event: EventRecord,
    ) -> WhippleScriptResult<Option<RunId>> {
        let policy = self
            .tasks
            .get(task_name)
            .map(|task| task.config.admission.when_busy.clone())
            .ok_or_else(|| WhippleScriptError::not_found(format!("task {task_name:?} was not found")))?;
    ...
    fn queue_pending(&mut self, task_name: &str, event: EventRecord) -> WhippleScriptResult<()> {
        let task = self.task_mut(task_name)?;
        task.pending.push_back(PendingTrigger { event });
        Ok(())
    }

    fn maybe_start_next_pending(&mut self, task_name: &str) -> WhippleScriptResult<()> {
        let event = {
            let task = self.task_mut(task_name)?;
    ...
    fn spawn_service(&mut self, name: &str) -> WhippleScriptResult<()> {
        let service = self.service(name)?;
    ...
    fn spawn_health_check(&mut self, service_name: &str) -> WhippleScriptResult<()> {
        let config = self.service(service_name)?.config.clone();
        let health_config = config.health.as_ref().ok_or_else(|| {
            WhippleScriptError::invalid_state(format!(
    ...
    fn cancel_run(&mut self, run_id: &str, reason: StopReason) -> WhippleScriptResult<()> {
        let managed = self
            .active_runs
            .get_mut(run_id)
            .ok_or_else(|| WhippleScriptError::not_found(format!("run {run_id} was not found")))?;
    ...
    fn stop_all_runs(&mut self, reason: StopReason) -> WhippleScriptResult<()> {
    ...
    fn poll_runs(&mut self) -> WhippleScriptResult<()> {
        let run_ids = self.active_runs.keys().cloned().collect::<Vec<_>>();
        for run_id in run_ids {
            ...
            if let Some(status) = finished {
                let managed = self
                    .active_runs
                    .remove(&run_id)
                    .ok_or_else(|| WhippleScriptError::internal("run disappeared during poll"))?;
    ...
    fn finish_run(&mut self, managed: ManagedRun, status: ExitStatus) -> WhippleScriptResult<()> {
    ...
    fn service(&self, name: &str) -> WhippleScriptResult<&ManagedService> {
        self.services
            .get(name)
            .ok_or_else(|| WhippleScriptError::not_found(format!("service {name:?} was not found")))
    }

    fn service_mut(&mut self, name: &str) -> WhippleScriptResult<&mut ManagedService> {
        self.services
            .get_mut(name)
            .ok_or_else(|| WhippleScriptError::not_found(format!("service {name:?} was not found")))
    }

    fn task_mut(&mut self, name: &str) -> WhippleScriptResult<&mut ManagedTask> {
        self.tasks
            .get_mut(name)
            .ok_or_else(|| WhippleScriptError::not_found(format!("task {name:?} was not found")))
    }
...
fn recover_unfinished_runs(store: &SqliteStore) -> WhippleScriptResult<()> {
...
fn append_recovery_note(path: &Path, run: &RunRecord) -> WhippleScriptResult<()> {
    ...
    writeln!(
        file,
        "[whipplescript] {} recovery: run {} marked failed during daemon startup; previous state was {:?}; no exit code or signal was observed",
        Utc::now().to_rfc3339(),
        run.id.as_str(),
        run.state
    )?;
...
fn refresh_run_meta(store: &SqliteStore, run_id: &RunId, kind: &str) -> WhippleScriptResult<()> {
...
fn bind_listener(path: impl AsRef<Path>) -> WhippleScriptResult<UnixListener> {
    UnixListener::bind(path)
        .map_err(|error| WhippleScriptError::internal(format!("failed to bind daemon socket: {error}")))
}

fn wait_for_startup(
    startup_rx: &mpsc::Receiver<WhippleScriptResult<()>>,
    join_handle: &JoinHandle<WhippleScriptResult<()>>,
    path: &Path,
    timeout: Duration,
) -> WhippleScriptResult<()> {
    match startup_rx.recv_timeout(timeout) {
        Ok(result) => result,
        Err(mpsc::RecvTimeoutError::Timeout) => Err(WhippleScriptError::unavailable(format!(
        ...
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            if join_handle.is_finished() {
                Err(WhippleScriptError::internal(
...
fn acquire_lock(path: &Path, pid: u32) -> WhippleScriptResult<()> {
...
fn lock_owner_id(owner_pid: u32) -> String {
    std::env::var("WHIPPLESCRIPT_RUN_ID").unwrap_or_else(|_| format!("pid:{owner_pid}"))
}

fn read_json_line<T: DeserializeOwned>(stream: &mut UnixStream) -> WhippleScriptResult<T> {
    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    reader.read_line(&mut line)?;
    serde_json::from_str(line.trim_end())
        .map_err(|error| WhippleScriptError::invalid_input(error.to_string()))
}

fn write_json_line<T: serde::Serialize>(stream: &mut UnixStream, value: &T) -> WhippleScriptResult<()> {
    let encoded =
...
fn run_envs(
    ...
    let mut envs = vec![
        ("WHIPPLESCRIPT_KIND".to_string(), kind.to_string()),
        ("WHIPPLESCRIPT_NAME".to_string(), record.name.clone()),
        (
            "WHIPPLESCRIPT_WORKSPACE".to_string(),
            runtime_paths.workspace_root().display().to_string(),
        ),
        (
            "WHIPPLESCRIPT_WORKSPACE_ROOT".to_string(),
            runtime_paths.workspace_root().display().to_string(),
        ),
        (
            "WHIPPLESCRIPT_CONFIG_DIR".to_string(),
            workspace.config_dir().display().to_string(),
        ),
        (
            "WHIPPLESCRIPT_STATE_DIR".to_string(),
            runtime_paths.state_root().display().to_string(),
        ),
        (
            "WHIPPLESCRIPT_RUN_ID".to_string(),
            record.id.as_str().to_string(),
        ),
        (
            "WHIPPLESCRIPT_CONFIG_VERSION".to_string(),
            config_version.to_string(),
        ),
    ];
    if let Some(run_directory) = &record.run_directory {
        envs.push(("WHIPPLESCRIPT_RUN_DIR".to_string(), run_directory.clone()));
    }
    if let Some(stdout_path) = &record.stdout_path {
        envs.push(("WHIPPLESCRIPT_STDOUT_LOG".to_string(), stdout_path.clone()));
    }
    if let Some(stderr_path) = &record.stderr_path {
        envs.push(("WHIPPLESCRIPT_STDERR_LOG".to_string(), stderr_path.clone()));
    }
    if let Some(restart_of) = &record.restart_of {
        envs.push((
            "WHIPPLESCRIPT_RESTART_OF".to_string(),
            restart_of.as_str().to_string(),
        ));
    }
    if let Some(attempt) = record.attempt {
        envs.push(("WHIPPLESCRIPT_RESTART_ATTEMPT".to_string(), attempt.to_string()));
    }
    if let Some(event) = event {
        envs.push((
            "WHIPPLESCRIPT_EVENT_ID".to_string(),
            event.id.as_str().to_string(),
        ));
        envs.push(("WHIPPLESCRIPT_EVENT_TYPE".to_string(), event.event_type.clone()));
        envs.push((
            "WHIPPLESCRIPT_EVENT_JSON".to_string(),
            serde_json::to_string(event).unwrap_or_else(|_| "{}".to_string()),
        ));
        let payload_json =
            serde_json::to_string(&event.payload).unwrap_or_else(|_| "null".to_string());
        envs.push((
            "WHIPPLESCRIPT_EVENT_PAYLOAD_JSON".to_string(),
            payload_json.clone(),
        ));
        envs.push(("WHIPPLESCRIPT_PAYLOAD_JSON".to_string(), payload_json));
        envs.push((
            "WHIPPLESCRIPT_EVENT_PATH".to_string(),
            event_path.display().to_string(),
        ));
        if let Some(correlation_id) = &event.correlation_id {
            envs.push((
                "WHIPPLESCRIPT_CORRELATION_ID".to_string(),
...
fn write_run_meta(path: &Path, record: &RunRecord, kind: &str) -> WhippleScriptResult<()> {
...
fn write_event_artifact(path: &Path, event: &EventRecord) -> WhippleScriptResult<()> {
    fs::write(
        path,
...
fn validate_dynamic_name(kind: &str, name: &str) -> WhippleScriptResult<()> {
...
fn validate_nonempty_dynamic(kind: &str, value: &str) -> WhippleScriptResult<()> {
    if value.trim().is_empty() {
        return Err(WhippleScriptError::invalid_input(format!("{kind} is required")));
    }
    Ok(())
}

fn optional_duration(value: Option<&str>) -> WhippleScriptResult<Option<Duration>> {
    value.map(parse_duration).transpose()
}

fn resolve_run_cwd(workspace_root: &Path, cwd: Option<PathBuf>) -> WhippleScriptResult<PathBuf> {
...
fn schedule_restart(
    ...
    let next_delay = match service.config.supervision.backoff {
        Some(whipplescript_core::BackoffMode::Exponential) => {
...
fn parse_schedule(expression: &str) -> WhippleScriptResult<Schedule> {
    ...
    Schedule::from_str(&normalized).map_err(|error| {
        WhippleScriptError::invalid_input(format!("invalid schedule {expression:?}: {error}"))
    })
}
...
fn scan_watch_patterns(
...
) -> WhippleScriptResult<Vec<PathBuf>> {
    ...
    for pattern in patterns {
        ...
        let entries = glob::glob(pattern_str).map_err(|error| {
            WhippleScriptError::invalid_input(format!("invalid watch pattern: {error}"))
        })?;
        for entry in entries {
            let path = entry.map_err(|error| WhippleScriptError::internal(error.to_string()))?;
...
"#,
        ...
        fs::write(
            fixture.root().join(".whipplescript/project.whip"),
            "[[service]]\nname = \"ticker\"\nrun =\n",
        )
        .unwrap();
...
run = "printf '%s' \"$WHIPPLESCRIPT_EVENT_TYPE\" > event-type.txt; printf '%s' \"$WHIPPLESCRIPT_EVENT_ID\" > event-id.txt; printf '%s' \"$WHIPPLESCRIPT_EVENT_PATH\" > event-path.txt; printf '%s' \"$WHIPPLESCRIPT_EVENT_JSON\" > event-json.txt; printf '%s' \"$WHIPPLESCRIPT_EVENT_PAYLOAD_JSON\" > event-payload-json.txt; printf '%s' \"$WHIPPLESCRIPT_PAYLOAD_JSON\" > payload-json.txt; printf '%s' \"$WHIPPLESCRIPT_WORKSPACE\" > workspace.txt; printf '%s' \"$WHIPPLESCRIPT_WORKSPACE_ROOT\" > workspace-root.txt; printf '%s' \"$WHIPPLESCRIPT_CONFIG_DIR\" > config-dir.txt; printf '%s' \"$WHIPPLESCRIPT_STATE_DIR\" > state-dir.txt; printf '%s' \"$WHIPPLESCRIPT_RUN_DIR\" > run-dir.txt; printf '%s' \"$WHIPPLESCRIPT_CONFIG_VERSION\" > config-version.txt; cp \"$WHIPPLESCRIPT_EVENT_PATH\" event-copy.json"
"#,
        );
        let workspace = discover_workspace(fixture.root()).unwrap();
        let handle = DaemonServer::start_with_options(
        ...
        assert_eq!(
            ...
            fixture.root().join(".whipplescript").display().to_string()
        );
        assert!(
...
"#,
        ...
        assert!(events
            .iter()
            .any(|event| event.routing == whipplescript_core::EventRouting::Schedule));
        assert!(events
            .iter()
            .any(|event| event.routing == whipplescript_core::EventRouting::Watch));
    ...
    impl Fixture {
        fn new(config: &str) -> Self {
            ...
            fs::create_dir_all(root_dir.path().join(".whipplescript")).unwrap();
            fs::write(
                root_dir.path().join(".whipplescript/project.whip"),
                config.trim(),
            )
            .unwrap();
