...
use crate::{WhippleScriptError, WhippleScriptResult, RunId, Workspace, WorkspaceId};

pub const DEFAULT_STATE_HOME_SUFFIX: &str = ".local/state";
pub const WHIPPLESCRIPT_STATE_DIR_NAME: &str = "whipplescript";
pub const WORKSPACES_STATE_DIR_NAME: &str = "workspaces";
pub const RUNS_DIR_NAME: &str = "runs";
pub const DATABASE_FILE_NAME: &str = "whipplescript.sqlite";
...
impl WorkspaceRuntimePaths {
    pub fn for_workspace(workspace: &Workspace) -> WhippleScriptResult<Self> {
    ...
    pub fn for_workspace_with_state_home(
        ...
        let state_root = state_home
            .as_ref()
            .join(WHIPPLESCRIPT_STATE_DIR_NAME)
    ...
    pub fn ensure_state_root(&self) -> WhippleScriptResult<()> {
        fs::create_dir_all(&self.state_root)?;
        Ok(())
    }

    pub fn prepare_run_directory(&self, run_id: &RunId) -> WhippleScriptResult<RunPaths> {
...
pub fn state_home_from_env() -> WhippleScriptResult<PathBuf> {
    ...
    let home = home
        ...
        .ok_or_else(|| {
            WhippleScriptError::invalid_input(
                "unable to resolve WhippleScript state home: set XDG_STATE_HOME or HOME",
            )
        })?;
...
mod tests {
    ...
    use super::{
        resolve_state_home, WorkspaceRuntimePaths, WHIPPLESCRIPT_STATE_DIR_NAME, DATABASE_FILE_NAME,
    ...
    fn uses_xdg_state_home_when_available() {
        let state_home = resolve_state_home(
            Some(PathBuf::from("/tmp/whipplescript-state").into_os_string()),
            None,
        )
        .unwrap();
        assert_eq!(state_home, PathBuf::from("/tmp/whipplescript-state"));
    }
    ...
    fn runtime_paths_separate_state_root_from_run_artifacts() {
        ...
        assert!(paths.state_root().components().any(|component| {
            component.as_os_str() == WHIPPLESCRIPT_STATE_DIR_NAME
