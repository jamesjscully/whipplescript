...
use crate::error::{WhippleScriptError, WhippleScriptResult};

fn validate_prefixed_ulid(value: &str, prefix: &str) -> WhippleScriptResult<()> {
    let ulid = value
        .strip_prefix(prefix)
        .ok_or_else(|| WhippleScriptError::invalid_input(format!("expected {prefix}<ulid>")))?;
    Ulid::from_string(ulid)
        .map(|_| ())
        .map_err(|_| WhippleScriptError::invalid_input(format!("invalid ULID in {value}")))
}
...
macro_rules! prefixed_id {
    ($name:ident, $prefix:literal) => {
        ...
        impl $name {
            ...
            pub fn parse(value: impl Into<String>) -> WhippleScriptResult<Self> {
...
impl WorkspaceId {
    pub fn from_canonical_path(path: &Path) -> WhippleScriptResult<Self> {
