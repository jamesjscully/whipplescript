...
impl Sandbox {
    ...
    fn armature_with_stdin<I, S>(&self, args: I, stdin: &str) -> Output
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
    ...
    where
        ...
        S: AsRef<OsStr>,
    {
        let output = self.armature_with_stdin(args, stdin);
        assert!(
...
fn event_service_lock_and_shutdown_flow_survives_real_cli_boundaries() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf %s "$ARMATURE_EVENT_TYPE" > event-type.txt
        printf %s "${{ARMATURE_CORRELATION_ID:-}}" > event-correlation.txt
        printf %s "$ARMATURE_EVENT_PAYLOAD_JSON" > event-payload.json
        printf %s "$ARMATURE_PAYLOAD_JSON" > payload.json
        printf %s "$ARMATURE_WORKSPACE" > workspace.txt
        printf %s "$ARMATURE_WORKSPACE_ROOT" > workspace-root.txt
        printf %s "$ARMATURE_CONFIG_DIR" > config-dir.txt
        printf %s "$ARMATURE_STATE_DIR" > state-dir.txt
        printf %s "$ARMATURE_RUN_DIR" > run-dir.txt
    ...
    assert_eq!(
        ...
        sandbox.root().join(".armature").display().to_string()
    );
...
fn manual_run_cancel_logs_and_runs_are_coherent_from_the_cli() {
    ...
    sandbox.write_script(
        ...
        printf %s "$ARMATURE_RUN_ID" > active-run-id.txt
...
fn daemon_restart_marks_previously_active_runs_as_failed_and_inspectable() {
    ...
    wait_until(
        || !sandbox.armature(["status"]).status.success(),
...
fn invalid_reload_preserves_running_daemon_and_valid_config_can_reload() {
    ...
    sandbox.write_file(".armature/armature.toml", good_config);
    sandbox.ok(["up"]);
    sandbox.ok(["down"]);
}
...
fn trigger_delivery_semantics_are_inspectable_from_the_cli() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf '%s %s\n' "$ARMATURE_EVENT_ID" "$ARMATURE_EVENT_PAYLOAD_JSON" >> "$1"
        sleep "$2"
        "#,
    );
...
fn dynamic_service_lifecycle() {
    ...
    let original_config = sandbox.read(".armature/armature.toml");
    sandbox.write_script(
        "dynamic-service.sh",
        r#"
        #!/bin/sh
        set -eu
        printf '%s\n' "${MARKER:-missing}:${ARMATURE_NAME}:${ARMATURE_KIND}" >> dynamic-started.log
        trap "printf '%s\n' \"${MARKER:-missing}:${ARMATURE_NAME}\" >> dynamic-stopped.log; exit 0" TERM INT
        while true; do sleep 1; done
        "#,
    );
    ...
    assert_eq!(sandbox.read(".armature/armature.toml"), original_config);
...
fn dynamic_task_event_and_watch_lifecycle() {
    ...
    sandbox.write_script(
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s|%s|%s\n' "$ARMATURE_NAME" "$ARMATURE_KIND" "${MARKER:-missing}" "$ARMATURE_EVENT_PAYLOAD_JSON" >> dynamic-event.log
        "#,
    );
    sandbox.write_script(
        "dynamic-watch.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s\n' "$ARMATURE_NAME" "$ARMATURE_EVENT_PAYLOAD_JSON" >> dynamic-watch.log
        "#,
    );
    sandbox.write_script(
        "dynamic-schedule.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s|%s\n' "$ARMATURE_NAME" "$ARMATURE_EVENT_TYPE" >> dynamic-schedule.log
        "#,
    );
...
fn wait_and_subscribe_agent_flow() {
    ...
    sandbox.write_script(
        ...
        set -eu
        printf '%s' "$ARMATURE_RUN_ID" > active-run-id.txt
        printf '%s' "${ARMATURE_CORRELATION_ID:-}" > correlation.txt
        "#,
    );
...
#[test]
#[ignore = "opt-in stress coverage; run with `cargo test -p armature-cli --test e2e -- --ignored sustained_stress_many_events_watch_changes_and_services`"]
...
fn run_sustained_load_scenario(
    ...
    sandbox.write_script(
        ...
        set -eu
        sleep 0.02
        printf '%s\n' "$ARMATURE_EVENT_PAYLOAD_JSON" >> event-order.log
        "#,
    );
    sandbox.write_script(
        "record-watch.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s\n' "$ARMATURE_EVENT_PAYLOAD_JSON" >> watch-events.log
        "#,
    );
    sandbox.write_script(
        "service.sh",
        ...
        #!/bin/sh
        set -eu
        printf '%s\n' "$ARMATURE_NAME:$ARMATURE_RUN_ID" >> services-started.log
        trap "exit 0" TERM INT
        while true; do sleep 1; done
        "#,
    );
