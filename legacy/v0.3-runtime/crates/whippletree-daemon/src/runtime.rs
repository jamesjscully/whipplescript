...
use whippletree_core::{
    load_workspace_config, AdmissionConfig, AdmissionPolicy, WhippletreeConfig, WhippletreeError,
    WhippletreeResult, EventId, EventRecord, EventRouting, ProcessState, ResourcePolicy, RestartMode,
...
pub struct DaemonHandle {
    socket_path: PathBuf,
    join_handle: Option<JoinHandle<WhippletreeResult<()>>>,
}
...
impl DaemonServer {
    pub fn start(workspace: Workspace) -> WhippletreeResult<DaemonHandle> {
    ...
    ) -> WhippletreeResult<DaemonHandle> {
...
impl DaemonHandle {
    ...
    pub fn join(mut self) -> WhippletreeResult<()> {
        if let Some(handle) = self.join_handle.take() {
            return handle
                .join()
                .map_err(|_| WhippletreeError::internal("daemon thread panicked"))?;
        }
        Ok(())
    }
}
...
impl DaemonClient {
    ...
    pub fn inspect(&self) -> WhippletreeResult<InspectResponse> {
        match self.send(DaemonRequest::Inspect)? {
            ResponsePayload::Inspect(response) => Ok(response),
            _ => Err(WhippletreeError::internal("unexpected inspect response")),
        }
    }

    pub fn events(&self) -> WhippletreeResult<Vec<EventRecord>> {
        match self.send(DaemonRequest::Events)? {
            ResponsePayload::Events { events } => Ok(events),
            _ => Err(WhippletreeError::internal("unexpected events response")),
        }
    }

    pub fn triggers(&self) -> WhippletreeResult<Vec<TriggerRecord>> {
        match self.send(DaemonRequest::Triggers)? {
            ResponsePayload::Triggers { triggers } => Ok(triggers),
            _ => Err(WhippletreeError::internal("unexpected triggers response")),
        }
    }

    pub fn runs(&self) -> WhippletreeResult<Vec<RunRecord>> {
        match self.send(DaemonRequest::Runs)? {
            ResponsePayload::Runs { runs } => Ok(runs),
            _ => Err(WhippletreeError::internal("unexpected runs response")),
        }
    }

    pub fn start_task(&self, name: impl Into<String>) -> WhippletreeResult<RunId> {
    ...
    pub fn start_task_with_provenance(
        ...
        match self.send(DaemonRequest::StartTask {
            ...
            _ => Err(WhippletreeError::internal("unexpected task start response")),
        }
    }
    ...
    pub fn start_adhoc(
        ...
        match self.send(DaemonRequest::StartAdhoc {
            ...
            ResponsePayload::StartedRun { run_id } => Ok(run_id),
            _ => Err(WhippletreeError::internal("unexpected ad hoc start response")),
        }
    }
    ...
    pub fn cancel_run(&self, run_id: impl Into<String>) -> WhippletreeResult<()> {
    ...
    pub fn acquire_lock(
    ...
    ) -> WhippletreeResult<ManualLockRecord> {
        ...
        match self.send(DaemonRequest::LockAcquire {
            ...
            _ => Err(WhippletreeError::internal("unexpected lock acquire response")),
        }
    }
    ...
    pub fn renew_lock(
        ...
        match self.send(DaemonRequest::LockRenew {
            ...
            _ => Err(WhippletreeError::internal("unexpected lock renew response")),
        }
    }
    ...
    pub fn force_release_lock(
        ...
        match self.send(DaemonRequest::LockForceRelease {
            ...
            _ => Err(WhippletreeError::internal(
                "unexpected lock force-release response",
            )),
        }
    }

    pub fn locks(&self) -> WhippletreeResult<Vec<ManualLockRecord>> {
        match self.send(DaemonRequest::LockStatus)? {
            ResponsePayload::Locks { locks } => Ok(locks),
            _ => Err(WhippletreeError::internal("unexpected lock status response")),
        }
    }

    pub fn service_stop(&self, name: impl Into<String>) -> WhippletreeResult<()> {
        self.expect_empty(DaemonRequest::ServiceStop { name: name.into() })
    }

    pub fn service_start(&self, name: impl Into<String>) -> WhippletreeResult<()> {
        self.expect_empty(DaemonRequest::ServiceStart { name: name.into() })
    }

    pub fn service_restart(&self, name: impl Into<String>) -> WhippletreeResult<()> {
    ...
    pub fn service_remove(&self, name: impl Into<String>) -> WhippletreeResult<()> {
    ...
    pub fn task_remove(&self, name: impl Into<String>) -> WhippletreeResult<()> {
        self.expect_empty(DaemonRequest::TaskRemove { name: name.into() })
    }

    pub fn reload_config(&self) -> WhippletreeResult<()> {
        self.expect_empty(DaemonRequest::ReloadConfig)
    }

    pub fn shutdown(&self) -> WhippletreeResult<()> {
        self.expect_empty(DaemonRequest::Shutdown)
    }

    fn expect_empty(&self, request: DaemonRequest) -> WhippletreeResult<()> {
        match self.send(request)? {
            ResponsePayload::Empty => Ok(()),
            _ => Err(WhippletreeError::internal("unexpected daemon response")),
        }
    }

    fn send(&self, request: DaemonRequest) -> WhippletreeResult<ResponsePayload> {
        let mut stream = UnixStream::connect(&self.socket_path).map_err(|error| {
            WhippletreeError::unavailable(format!(
        ...
        match response {
            ...
            DaemonResponse::Error { kind, message } => Err(WhippletreeError {
                kind: kind.into(),
...
impl Runtime {
    fn new(
        ...
        runtime_paths: WorkspaceRuntimePaths,
        options: DaemonOptions,
    ) -> WhippletreeResult<Self> {
        let listener = bind_listener(runtime_paths.socket_path())?;
        listener
            .set_nonblocking(true)
            .map_err(|error| WhippletreeError::internal(error.to_string()))?;
    ...
    fn run(&mut self) -> WhippletreeResult<()> {
    ...
    fn accept_requests(&mut self) -> WhippletreeResult<()> {
        loop {
            match self.listener.accept() {
                ...
                Err(error) => return Err(WhippletreeError::internal(error.to_string())),
            }
        }
    }
    ...
    fn handle_request_inner(&mut self, request: DaemonRequest) -> WhippletreeResult<ResponsePayload> {
        if self.shutdown_requested && !request_allowed_during_shutdown(&request) {
            return Err(WhippletreeError::invalid_state("daemon is shutting down"));
        }
    ...
    fn start_task(
        ...
        if !self.tasks.contains_key(name) {
            return Err(WhippletreeError::not_found(format!(
        ...
        route_result.started_run_id.ok_or_else(|| {
            WhippletreeError::conflict(format!(
                "task {name:?} did not start immediately because admission policy queued or rejected it"
            ))
        })
    }

    fn start_adhoc(&mut self, request: AdhocRunRequest) -> WhippletreeResult<RunId> {
        if request.name.trim().is_empty() {
            return Err(WhippletreeError::invalid_input(
    ...
    fn reload_config(&mut self) -> WhippletreeResult<()> {
    ...
    fn apply_service_config_changes(&mut self, previous: &WhippletreeConfig) -> WhippletreeResult<()> {
    ...
    fn rebuild_runtime_state(&mut self) -> WhippletreeResult<()> {
    ...
    fn add_dynamic_service(&mut self, request: DynamicServiceRequest) -> WhippletreeResult<()> {
        ...
        {
            return Err(WhippletreeError::conflict(format!(
    ...
    fn remove_dynamic_service(&mut self, name: &str) -> WhippletreeResult<()> {
        let active_run_id = {
    ...
    fn add_dynamic_task(&mut self, request: DynamicTaskRequest) -> WhippletreeResult<()> {
    ...
    fn remove_dynamic_task(&mut self, name: &str) -> WhippletreeResult<()> {
        let task = self
            .tasks
            .get(name)
            .ok_or_else(|| WhippletreeError::not_found(format!("task {name:?} was not found")))?;
        if !task.dynamic {
            return Err(WhippletreeError::conflict(format!(
    ...
    fn rebuild_schedules(&mut self) -> WhippletreeResult<()> {
        let mut next = HashMap::new();
    ...
    fn rebuild_watches(&mut self) -> WhippletreeResult<()> {
        let mut next = HashMap::new();
        for task in self.tasks.values() {
    ...
    fn reconcile_services(&mut self) -> WhippletreeResult<()> {
        let now = Instant::now();
    ...
    fn poll_health_checks(&mut self) -> WhippletreeResult<()> {
        let now = Instant::now();
        let service_names = self.services.keys().cloned().collect::<Vec<_>>();
        for name in service_names {
            let should_check = {
    ...
    fn poll_schedules(&mut self) -> WhippletreeResult<()> {
        let now = Utc::now();
    ...
    fn poll_watches(&mut self) -> WhippletreeResult<()> {
        ...
        for task_name in task_names {
            ...
            let watch = self
                .watches
                .get_mut(&task_name)
                .ok_or_else(|| WhippletreeError::internal("watch state disappeared"))?;
    ...
    fn apply_admission(
        ...
        task_name: &str,
        event: EventRecord,
    ) -> WhippletreeResult<Option<RunId>> {
        let policy = self
            .tasks
            .get(task_name)
            .map(|task| task.config.admission.when_busy.clone())
            .ok_or_else(|| WhippletreeError::not_found(format!("task {task_name:?} was not found")))?;
    ...
    fn queue_pending(&mut self, task_name: &str, event: EventRecord) -> WhippletreeResult<()> {
        let task = self.task_mut(task_name)?;
        task.pending.push_back(PendingTrigger { event });
        Ok(())
    }

    fn maybe_start_next_pending(&mut self, task_name: &str) -> WhippletreeResult<()> {
        let event = {
            let task = self.task_mut(task_name)?;
    ...
    fn spawn_service(&mut self, name: &str) -> WhippletreeResult<()> {
        let service = self.service(name)?;
    ...
    fn spawn_health_check(&mut self, service_name: &str) -> WhippletreeResult<()> {
        let config = self.service(service_name)?.config.clone();
        let health_config = config.health.as_ref().ok_or_else(|| {
            WhippletreeError::invalid_state(format!(
    ...
    fn cancel_run(&mut self, run_id: &str, reason: StopReason) -> WhippletreeResult<()> {
        let managed = self
            .active_runs
            .get_mut(run_id)
            .ok_or_else(|| WhippletreeError::not_found(format!("run {run_id} was not found")))?;
    ...
    fn stop_all_runs(&mut self, reason: StopReason) -> WhippletreeResult<()> {
    ...
    fn poll_runs(&mut self) -> WhippletreeResult<()> {
        let run_ids = self.active_runs.keys().cloned().collect::<Vec<_>>();
        for run_id in run_ids {
            ...
            if let Some(status) = finished {
                let managed = self
                    .active_runs
                    .remove(&run_id)
                    .ok_or_else(|| WhippletreeError::internal("run disappeared during poll"))?;
    ...
    fn finish_run(&mut self, managed: ManagedRun, status: ExitStatus) -> WhippletreeResult<()> {
    ...
    fn service(&self, name: &str) -> WhippletreeResult<&ManagedService> {
        self.services
            .get(name)
            .ok_or_else(|| WhippletreeError::not_found(format!("service {name:?} was not found")))
    }

    fn service_mut(&mut self, name: &str) -> WhippletreeResult<&mut ManagedService> {
        self.services
            .get_mut(name)
            .ok_or_else(|| WhippletreeError::not_found(format!("service {name:?} was not found")))
    }

    fn task_mut(&mut self, name: &str) -> WhippletreeResult<&mut ManagedTask> {
        self.tasks
            .get_mut(name)
            .ok_or_else(|| WhippletreeError::not_found(format!("task {name:?} was not found")))
    }
...
fn recover_unfinished_runs(store: &SqliteStore) -> WhippletreeResult<()> {
...
fn append_recovery_note(path: &Path, run: &RunRecord) -> WhippletreeResult<()> {
    ...
    writeln!(
        file,
        "[whippletree] {} recovery: run {} marked failed during daemon startup; previous state was {:?}; no exit code or signal was observed",
        Utc::now().to_rfc3339(),
        run.id.as_str(),
        run.state
    )?;
...
fn refresh_run_meta(store: &SqliteStore, run_id: &RunId, kind: &str) -> WhippletreeResult<()> {
...
fn bind_listener(path: impl AsRef<Path>) -> WhippletreeResult<UnixListener> {
    UnixListener::bind(path)
        .map_err(|error| WhippletreeError::internal(format!("failed to bind daemon socket: {error}")))
}

fn wait_for_startup(
    startup_rx: &mpsc::Receiver<WhippletreeResult<()>>,
    join_handle: &JoinHandle<WhippletreeResult<()>>,
    path: &Path,
    timeout: Duration,
) -> WhippletreeResult<()> {
    match startup_rx.recv_timeout(timeout) {
        Ok(result) => result,
        Err(mpsc::RecvTimeoutError::Timeout) => Err(WhippletreeError::unavailable(format!(
...
fn acquire_lock(path: &Path, pid: u32) -> WhippletreeResult<()> {
...
fn lock_owner_id(owner_pid: u32) -> String {
    std::env::var("WHIPPLETREE_RUN_ID").unwrap_or_else(|_| format!("pid:{owner_pid}"))
}

fn read_json_line<T: DeserializeOwned>(stream: &mut UnixStream) -> WhippletreeResult<T> {
    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    reader.read_line(&mut line)?;
    serde_json::from_str(line.trim_end())
        .map_err(|error| WhippletreeError::invalid_input(error.to_string()))
}

fn write_json_line<T: serde::Serialize>(stream: &mut UnixStream, value: &T) -> WhippletreeResult<()> {
    let encoded =
        serde_json::to_string(value).map_err(|error| WhippletreeError::internal(error.to_string()))?;
...
fn run_envs(
    ...
    let mut envs = vec![
        ("WHIPPLETREE_KIND".to_string(), kind.to_string()),
        ("WHIPPLETREE_NAME".to_string(), record.name.clone()),
        (
            "WHIPPLETREE_WORKSPACE".to_string(),
            runtime_paths.workspace_root().display().to_string(),
        ),
        (
            "WHIPPLETREE_WORKSPACE_ROOT".to_string(),
            runtime_paths.workspace_root().display().to_string(),
        ),
        (
            "WHIPPLETREE_CONFIG_DIR".to_string(),
            workspace.config_dir().display().to_string(),
        ),
        (
            "WHIPPLETREE_STATE_DIR".to_string(),
            runtime_paths.state_root().display().to_string(),
        ),
        (
            "WHIPPLETREE_RUN_ID".to_string(),
            record.id.as_str().to_string(),
        ),
        (
            "WHIPPLETREE_CONFIG_VERSION".to_string(),
            config_version.to_string(),
        ),
    ];
    if let Some(run_directory) = &record.run_directory {
        envs.push(("WHIPPLETREE_RUN_DIR".to_string(), run_directory.clone()));
    }
    if let Some(stdout_path) = &record.stdout_path {
        envs.push(("WHIPPLETREE_STDOUT_LOG".to_string(), stdout_path.clone()));
    }
    if let Some(stderr_path) = &record.stderr_path {
        envs.push(("WHIPPLETREE_STDERR_LOG".to_string(), stderr_path.clone()));
    }
    if let Some(restart_of) = &record.restart_of {
        envs.push((
            "WHIPPLETREE_RESTART_OF".to_string(),
            restart_of.as_str().to_string(),
        ));
    }
    if let Some(attempt) = record.attempt {
        envs.push(("WHIPPLETREE_RESTART_ATTEMPT".to_string(), attempt.to_string()));
    }
    if let Some(event) = event {
        envs.push((
            "WHIPPLETREE_EVENT_ID".to_string(),
            event.id.as_str().to_string(),
        ));
        envs.push(("WHIPPLETREE_EVENT_TYPE".to_string(), event.event_type.clone()));
        envs.push((
            "WHIPPLETREE_EVENT_JSON".to_string(),
            serde_json::to_string(event).unwrap_or_else(|_| "{}".to_string()),
        ));
        let payload_json =
            serde_json::to_string(&event.payload).unwrap_or_else(|_| "null".to_string());
        envs.push((
            "WHIPPLETREE_EVENT_PAYLOAD_JSON".to_string(),
            payload_json.clone(),
        ));
        envs.push(("WHIPPLETREE_PAYLOAD_JSON".to_string(), payload_json));
        envs.push((
            "WHIPPLETREE_EVENT_PATH".to_string(),
            event_path.display().to_string(),
        ));
        if let Some(correlation_id) = &event.correlation_id {
            envs.push((
                "WHIPPLETREE_CORRELATION_ID".to_string(),
...
fn write_run_meta(path: &Path, record: &RunRecord, kind: &str) -> WhippletreeResult<()> {
...
fn write_event_artifact(path: &Path, event: &EventRecord) -> WhippletreeResult<()> {
    fs::write(
        path,
...
fn validate_dynamic_name(kind: &str, name: &str) -> WhippletreeResult<()> {
    if name.trim().is_empty() {
        return Err(WhippletreeError::invalid_input(format!(
    ...
    Err(WhippletreeError::invalid_input(format!(
        "{kind} name {name:?} may only contain ASCII letters, digits, '_', '-', '.', or ':'"
    )))
}

fn validate_nonempty_dynamic(kind: &str, value: &str) -> WhippletreeResult<()> {
    if value.trim().is_empty() {
        return Err(WhippletreeError::invalid_input(format!("{kind} is required")));
    }
    Ok(())
}

fn optional_duration(value: Option<&str>) -> WhippletreeResult<Option<Duration>> {
    value.map(parse_duration).transpose()
}

fn resolve_run_cwd(workspace_root: &Path, cwd: Option<PathBuf>) -> WhippletreeResult<PathBuf> {
...
fn schedule_restart(
    ...
    let next_delay = match service.config.supervision.backoff {
        Some(whippletree_core::BackoffMode::Exponential) => {
...
fn parse_schedule(expression: &str) -> WhippletreeResult<Schedule> {
    ...
    Schedule::from_str(&normalized).map_err(|error| {
        WhippletreeError::invalid_input(format!("invalid schedule {expression:?}: {error}"))
    })
}
...
fn scan_watch_patterns(
    ...
    for pattern in patterns {
        ...
        let entries = glob::glob(pattern_str).map_err(|error| {
            WhippletreeError::invalid_input(format!("invalid watch pattern: {error}"))
        })?;
        for entry in entries {
            let path = entry.map_err(|error| WhippletreeError::internal(error.to_string()))?;
...
"#,
        ...
        fs::write(
            fixture.root().join(".whippletree/project.whip"),
            "[[service]]\nname = \"ticker\"\nrun =\n",
        )
        .unwrap();
...
run = "printf '%s' \"$WHIPPLETREE_EVENT_TYPE\" > event-type.txt; printf '%s' \"$WHIPPLETREE_EVENT_ID\" > event-id.txt; printf '%s' \"$WHIPPLETREE_EVENT_PATH\" > event-path.txt; printf '%s' \"$WHIPPLETREE_EVENT_JSON\" > event-json.txt; printf '%s' \"$WHIPPLETREE_EVENT_PAYLOAD_JSON\" > event-payload-json.txt; printf '%s' \"$WHIPPLETREE_PAYLOAD_JSON\" > payload-json.txt; printf '%s' \"$WHIPPLETREE_WORKSPACE\" > workspace.txt; printf '%s' \"$WHIPPLETREE_WORKSPACE_ROOT\" > workspace-root.txt; printf '%s' \"$WHIPPLETREE_CONFIG_DIR\" > config-dir.txt; printf '%s' \"$WHIPPLETREE_STATE_DIR\" > state-dir.txt; printf '%s' \"$WHIPPLETREE_RUN_DIR\" > run-dir.txt; printf '%s' \"$WHIPPLETREE_CONFIG_VERSION\" > config-version.txt; cp \"$WHIPPLETREE_EVENT_PATH\" event-copy.json"
"#,
        );
        let workspace = discover_workspace(fixture.root()).unwrap();
        let handle = DaemonServer::start_with_options(
        ...
        assert_eq!(
            ...
            fixture.root().join(".whippletree").display().to_string()
        );
        assert!(
...
"#,
        ...
        assert!(events
            .iter()
            .any(|event| event.routing == whippletree_core::EventRouting::Schedule));
        assert!(events
            .iter()
            .any(|event| event.routing == whippletree_core::EventRouting::Watch));
    ...
    impl Fixture {
        fn new(config: &str) -> Self {
            ...
            fs::create_dir_all(root_dir.path().join(".whippletree")).unwrap();
            fs::write(
                root_dir.path().join(".whippletree/project.whip"),
                config.trim(),
            )
            .unwrap();
