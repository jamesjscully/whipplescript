...
use whippletree_core::{
    WhippletreeError, WhippletreeResult, EventId, EventRecord, LogRecord, ProcessState, RunId, RunOrigin,
...
impl ManualLockStore {
    ...
    pub fn release(&self, name: &str, token: &str) -> WhippletreeResult<()> {
        let path = self.lock_file_path(name);
    ...
    pub fn force_release(&self, name: &str) -> WhippletreeResult<ManualLockRecord> {
        let path = self.lock_file_path(name);
    ...
    pub fn list(&self) -> WhippletreeResult<Vec<ManualLockRecord>> {
    ...
    fn read_lock_record(&self, path: &Path, name: &str) -> WhippletreeResult<ManualLockRecord> {
        if !path.exists() {
            return Err(WhippletreeError::not_found(format!(
                "lock {:?} is not held",
                name
        ...
        let mut record: ManualLockRecord = serde_json::from_slice(&raw)
            .map_err(|error| WhippletreeError::internal(format!("invalid lock record: {error}")))?;
    ...
    fn write_new_lock(&self, path: &Path, record: &ManualLockRecord) -> WhippletreeResult<()> {
        ...
        let mut file = fs::OpenOptions::new()
            ...
            .map_err(|error| match error.kind() {
                std::io::ErrorKind::AlreadyExists => {
                    WhippletreeError::conflict(format!("lock {:?} is already held", record.name))
                }
                _ => WhippletreeError::internal(error.to_string()),
            })?;
        file.write_all(&contents)?;
        Ok(())
    }

    fn write_lock_replace(&self, path: &Path, record: &ManualLockRecord) -> WhippletreeResult<()> {
        let contents = serde_json::to_vec_pretty(record)
            .map_err(|error| WhippletreeError::internal(error.to_string()))?;
    ...
    fn read_lock_if_fresh(&self, path: &Path) -> WhippletreeResult<Option<ManualLockRecord>> {
        if !path.exists() {
            return Ok(None);
        }
...
impl SqliteStore {
    pub fn open(workspace: &Workspace) -> WhippletreeResult<Self> {
        let paths = WorkspaceRuntimePaths::for_workspace(workspace)?;
        Self::open_with_paths(paths)
    }

    pub fn open_with_paths(paths: WorkspaceRuntimePaths) -> WhippletreeResult<Self> {
    ...
    pub fn record_event(&self, event: &EventRecord) -> WhippletreeResult<()> {
    ...
    pub fn get_event(&self, event_id: &EventId) -> WhippletreeResult<Option<EventRecord>> {
        self.connection
            .query_row(
    ...
    pub fn list_events(&self) -> WhippletreeResult<Vec<EventRecord>> {
    ...
    pub fn list_events_filtered(&self, filter: &EventFilter) -> WhippletreeResult<Vec<EventRecord>> {
    ...
    pub fn create_run_record(&self, new_run: NewRun) -> WhippletreeResult<PreparedRun> {
    ...
    pub fn record_run(&self, run: &RunRecord) -> WhippletreeResult<()> {
        let now = now_millis();
    ...
    pub fn update_run_state(&self, run_id: &RunId, state: ProcessState) -> WhippletreeResult<()> {
        let updated = self
            .connection
            .execute(
                "UPDATE runs
    ...
    pub fn get_run(&self, run_id: &RunId) -> WhippletreeResult<Option<RunRecord>> {
        self.connection
            .query_row(
    ...
    pub fn list_runs(&self) -> WhippletreeResult<Vec<RunRecord>> {
        let mut statement = self
            .connection
            .prepare(
                "SELECT id, name, command, origin, state, start_time, end_time, exit_code, signal,
    ...
    pub fn list_runs_filtered(&self, filter: &RunFilter) -> WhippletreeResult<Vec<RunRecord>> {
    ...
    pub fn list_unfinished_runs(&self) -> WhippletreeResult<Vec<RunRecord>> {
        Ok(self
            .list_runs()?
            .into_iter()
            .filter(|run| {
    ...
    pub fn record_logs(&self, logs: &LogRecord) -> WhippletreeResult<()> {
        self.connection
            .execute(
    ...
    pub fn get_logs(&self, run_id: &RunId) -> WhippletreeResult<Option<LogRecord>> {
        self.connection
            .query_row(
    ...
    pub fn record_trigger(&self, trigger: &TriggerRecord) -> WhippletreeResult<()> {
        self.connection
            .execute(
    ...
    pub fn list_triggers(&self) -> WhippletreeResult<Vec<TriggerRecord>> {
        let mut statement = self
            .connection
            .prepare(
    ...
    ) -> WhippletreeResult<Vec<TriggerRecord>> {
    ...
    fn bootstrap(&self) -> WhippletreeResult<()> {
        self.connection
            .execute_batch(
        ...
        if schema_version > SCHEMA_VERSION {
            return Err(WhippletreeError::internal(format!(
    ...
    fn schema_version(&self) -> WhippletreeResult<i32> {
        self.connection
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .map_err(map_sqlite_error)
    }

    fn set_schema_version(&self, version: i32) -> WhippletreeResult<()> {
        self.connection
            .execute_batch(&format!("PRAGMA user_version = {version}"))
            .map_err(map_sqlite_error)
    }

    fn migrate_schema(&self, from_version: i32) -> WhippletreeResult<()> {
        if from_version < 1 {
            self.migrate_to_1()?;
        }
        if from_version < 2 {
    ...
    fn migrate_to_1(&self) -> WhippletreeResult<()> {
    ...
    fn migrate_to_2(&self) -> WhippletreeResult<()> {
    ...
    fn ensure_events_column(&self, name: &str, definition: &str) -> WhippletreeResult<()> {
        self.ensure_column("events", name, definition)
    }

    fn ensure_runs_column(&self, name: &str, definition: &str) -> WhippletreeResult<()> {
        self.ensure_column("runs", name, definition)
    }

    fn ensure_column(&self, table: &str, name: &str, definition: &str) -> WhippletreeResult<()> {
        let mut statement = self
            .connection
...
fn enum_to_sql<T>(value: &T) -> WhippletreeResult<String>
where
    T: Serialize,
{
    let encoded =
...
fn enum_from_sql<T>(value: &str) -> WhippletreeResult<T>
where
...
fn map_sqlite_error(error: rusqlite::Error) -> WhippletreeError {
    WhippletreeError::internal(error.to_string())
}

fn to_sqlite_user_error(error: WhippletreeError) -> rusqlite::Error {
...
mod tests {
    ...
    use whippletree_core::{discover_workspace, EventRouting, TriggerOutcome};
    ...
    fn bootstraps_database_under_state_root() {
        ...
        assert_eq!(
            ...
            &fixture.root().join(".whippletree").join("runs")
        );
    ...
    fn upgrades_legacy_schema_and_preserves_records() {
        ...
        let event_id = whippletree_core::EventId::new();
        let run_id = whippletree_core::RunId::new();
    ...
    fn create_run_persists_isolated_paths_inside_workspace() {
        ...
        assert!(prepared
            .paths
            .directory
            .starts_with(fixture.root().join(".whippletree/runs")));
        assert_eq!(
    ...
    fn records_and_reads_back_events_and_runs() {
        ...
        let event = EventRecord {
            id: whippletree_core::EventId::new(),
    ...
    fn records_trigger_outcomes_for_inspection() {
        ...
        let trigger = TriggerRecord {
            ...
            routing: EventRouting::Watch,
            admission: whippletree_core::AdmissionPolicy::QueueOne,
