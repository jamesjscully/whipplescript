...
use whippletree_core::{
    load_workspace_config, resolve_workspace, WhippletreeConfig, WhippletreeError, WhippletreeResult,
    EventId, ProcessState, RestartMode, RunId, Workspace, WorkspaceRuntimePaths, CONFIG_DIR_NAME,
    CONFIG_FILE_NAME,
};
use whippletree_daemon::{
...
const INIT_TEMPLATE: &str = "# Whippletree v0.3 config\n#\n# Add tasks and services here.\n#\n# [[task]]\n# name = \"example\"\n# run = \"echo hello from whippletree\"\n";
const RECIPE_FILE_WATCH_TESTS_CONFIG: &str = "# Whippletree v0.3 recipe: file-watch tests\n# Edit paths and commands to match your project.\n\n[[task]]\nname = \"test-on-change\"\nwatch = [\"src/**/*\", \"tests/**/*\"]\nrun = \"./scripts/run-tests.sh\"\n";
...
impl Command {
    fn execute(self, workspace: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
        match self {
...
impl InternalCommand {
    fn execute(self) -> WhippletreeResult<()> {
        match self {
...
#[command(
    ...
    after_help = "Starts one finite ad hoc command through the daemon. Payload input defaults to an empty JSON object. Use -- to separate Whippletree options from the command."
)]
struct AdhocRunArgs {
    #[arg(long)]
    name: String,
...
#[derive(Debug, Serialize)]
struct PsOutput {
    runs: Vec<whippletree_core::RunRecord>,
}
...
struct OverviewOutput {
    ...
    recent_events: Vec<whippletree_core::EventRecord>,
    recent_triggers: Vec<whippletree_core::TriggerRecord>,
...
struct LogsOutput {
    run_id: String,
    run: Option<whippletree_core::RunRecord>,
...
fn resolve_init_root(workspace_arg: Option<PathBuf>) -> WhippletreeResult<PathBuf> {
    let root = workspace_arg.unwrap_or(std::env::current_dir()?);
    if root.exists() {
...
fn make_executable(path: &Path) -> WhippletreeResult<()> {
...
fn dev_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
...
fn down_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
...
fn parse_run_alias(raw: Vec<OsString>) -> WhippletreeResult<RunArgs> {
    let mut values = raw.into_iter();
    let task_name = values
        .next()
        .and_then(|value| value.into_string().ok())
        .ok_or_else(|| WhippletreeError::invalid_input("expected task name"))?;
    let mut correlation = None;
    while let Some(value) = values.next() {
        let value = value
            .into_string()
            .map_err(|_| WhippletreeError::invalid_input("invalid run argument"))?;
        if value == "--correlation" {
            let next = values
                .next()
                .and_then(|value| value.into_string().ok())
                .ok_or_else(|| WhippletreeError::invalid_input("--correlation requires a value"))?;
...
fn read_emit_payload(args: &EmitArgs) -> WhippletreeResult<(Value, String)> {
    ...
    if let Some(path) = args.payload_file.as_deref() {
        let raw = fs::read_to_string(path).map_err(|error| {
            WhippletreeError::invalid_input(format!(
    ...
    if args.stdin {
        ...
        io::stdin().read_to_string(&mut raw).map_err(|error| {
            WhippletreeError::invalid_input(format!("failed to read JSON payload from stdin: {error}"))
        })?;
...
fn read_adhoc_payload(args: &AdhocRunArgs) -> WhippletreeResult<(Value, String)> {
    if let Some(raw) = args.json.as_deref() {
        return parse_emit_payload(raw, "--json").map(|payload| (payload, "json".to_string()));
    }
...
fn parse_env_pairs(raw: &[String]) -> WhippletreeResult<Vec<(String, String)>> {
    raw.iter()
        .map(|pair| {
...
fn provenance_from_env(correlation: Option<String>) -> WhippletreeResult<EventProvenance> {
    let source_run_id = optional_env("WHIPPLETREE_RUN_ID")
        .map(RunId::parse)
        .transpose()?;
    let parent_event_id = optional_env("WHIPPLETREE_EVENT_ID")
        .map(EventId::parse)
        .transpose()?;
    let correlation_id = correlation
        .or_else(|| optional_env("WHIPPLETREE_CORRELATION_ID"))
...
fn parse_emit_payload(raw: &str, source: &str) -> WhippletreeResult<Value> {
    serde_json::from_str::<Value>(raw).map_err(|error| {
        WhippletreeError::invalid_input(format!("invalid JSON payload from {source}: {error}"))
    })
}
            ...
            .map(|event| {
                format!(
                    ...
                    event
                        .source
                        .as_deref()
                        .unwrap_or(whippletree_core::model::DEFAULT_EVENT_SOURCE)
                )
            })
            .collect::<Vec<_>>()
            .join("\n")
...
fn event_show(
    ...
    let event = store.get_event(&event_id)?.ok_or_else(|| {
        WhippletreeError::not_found(format!("event {} was not found", event_id.as_str()))
    })?;
    let text = format!(
        "{}\t{}\t{}",
        event.id.as_str(),
...
fn trigger_show(
    ...
    let trigger = store
        ...
        .ok_or_else(|| {
            WhippletreeError::not_found(format!("trigger {:?} was not found", args.trigger_id))
        })?;
    let text = format!(
        "{}\t{}\t{}\t{:?}",
        trigger.id.as_str(),
...
fn subscribe_events(workspace_arg: Option<PathBuf>) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
...
fn subscribe_runs(workspace_arg: Option<PathBuf>) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
    let mut seen = SqliteStore::open(&workspace)?
        .list_runs()?
        .into_iter()
        .map(|run| Ok(format!("{}:{}", run.id.as_str(), enum_text(&run.state)?)))
        .collect::<WhippletreeResult<HashSet<_>>>()?;
    loop {
        std::thread::sleep(Duration::from_millis(100));
...
fn subscribe_triggers(workspace_arg: Option<PathBuf>) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
    let mut seen = SqliteStore::open(&workspace)?
        .list_triggers()?
        .into_iter()
...
fn wait_until_observed<T>(
    timeout: Duration,
    label: &str,
    mut observe: impl FnMut() -> WhippletreeResult<Option<T>>,
) -> WhippletreeResult<T> {
    ...
    Err(WhippletreeError::unavailable(format!(
        "timed out waiting for {label}"
    )))
}

fn write_ndjson<T: Serialize>(value: &T) -> WhippletreeResult<()> {
...
fn status_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
    let inspect = daemon_client(&workspace).inspect()?;
    ...
    let tasks = task_views
        ...
        .map(|task| {
            let latest_run = runs
                .iter()
                .find(|run| {
                    run.name == task.name && matches!(run.origin, whippletree_core::RunOrigin::Task)
                })
                .map(run_summary);
            let latest_failure = runs
                ...
                .find(|run| {
                    run.name == task.name
                        && matches!(run.origin, whippletree_core::RunOrigin::Task)
                        && run_failed(run)
                })
                .map(run_summary);
            TaskOverview {
            ...
            let latest_run = runs
                ...
                .find(|run| {
                    run.name == service.name
                        && matches!(
                            run.origin,
                            whippletree_core::RunOrigin::Service | whippletree_core::RunOrigin::Restart
                        )
                })
                .map(run_summary);
            let latest_failure = runs
...
fn ps_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
    let inspect = daemon_client(&workspace).inspect()?;
    let output = PsOutput {
...
fn tasks_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
...
fn task_show(
    ...
    let task = build_task_views(&config, Some(&inspect))
        ...
        .ok_or_else(|| WhippletreeError::not_found(format!("task {:?} was not found", args.name)))?;
    let text = format!(
        "{}\nrun {}\nadmission {}\nactive_runs {}",
        task.name,
        task.run,
...
fn services_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
...
fn service_show(
    ...
    let service = build_service_views(&config, Some(&inspect))
        ...
        .ok_or_else(|| {
            WhippletreeError::not_found(format!("service {:?} was not found", args.name))
        })?;
    let text = format!(
        "{}\nrun {}\nenabled {}\nstate {}",
        service.name,
...
fn run_show(
    ...
    let run = store.get_run(&run_id)?.ok_or_else(|| {
        WhippletreeError::not_found(format!("run {} was not found", run_id.as_str()))
    })?;
    let text = format!(
        "{}\t{}\t{:?}\t{:?}",
        run.id.as_str(),
...
fn logs_workspace(
    ...
    let logs = store.get_logs(&run_id)?.ok_or_else(|| {
        WhippletreeError::not_found(format!("logs for {} were not found", run_id.as_str()))
    })?;
...
fn doctor_workspace(workspace_arg: Option<PathBuf>, format: OutputFormat) -> WhippletreeResult<()> {
    let workspace = resolve_workspace_arg(workspace_arg)?;
    ...
    match command {
        LockCommand::Acquire(args) => {
            let ttl = args
                ...
                .as_deref()
                .ok_or_else(|| {
                    WhippletreeError::invalid_input(
        ...
        LockCommand::Show(args) => {
            let lock = client
                ...
                .ok_or_else(|| {
                    WhippletreeError::not_found(format!("lock {:?} is not held", args.name))
                })?;
...
fn print_lock_show(format: OutputFormat, lock: &ManualLockRecord) -> WhippletreeResult<()> {
    let text = format!(
        "{}\nowner {}\npid {}\nexpires_at {}\nreason {}",
        lock.name,
        lock.owner_id,
...
fn print_lock_list(format: OutputFormat, locks: &[ManualLockRecord]) -> WhippletreeResult<()> {
...
) -> WhippletreeResult<std::process::ExitStatus> {
    let Some((program, args)) = command.split_first() else {
        return Err(WhippletreeError::invalid_input("lock with requires a command"));
    };
    ProcessCommand::new(program)
        .args(args)
        .env("WHIPPLETREE_LOCK_NAME", &lock.name)
        .env("WHIPPLETREE_LOCK_TOKEN", &lock.token)
        .env("WHIPPLETREE_LOCK_OWNER_ID", &lock.owner_id)
        .status()
        .map_err(|error| WhippletreeError::internal(format!("failed to run locked command: {error}")))
}

fn list_expired_locks(workspace: &Workspace) -> WhippletreeResult<Vec<ManualLockRecord>> {
    let runtime_paths = WorkspaceRuntimePaths::for_workspace(workspace)?;
    ...
    for entry in fs::read_dir(lock_dir)? {
        ...
        let record = serde_json::from_slice::<ManualLockRecord>(&raw).map_err(|error| {
            WhippletreeError::internal(format!("invalid lock record {}: {error}", path.display()))
        })?;
        if record
            .expires_at_ms
...
fn serve_workspace(workspace_root: PathBuf) -> WhippletreeResult<()> {
    let workspace = resolve_workspace(Some(&workspace_root), &workspace_root)?;
    let handle = DaemonServer::start(workspace)?;
    handle.join()
}

fn resolve_workspace_arg(workspace_arg: Option<PathBuf>) -> WhippletreeResult<Workspace> {
...
fn install_shutdown_handler(client: DaemonClient) -> WhippletreeResult<()> {
    let (sender, receiver) = mpsc::channel::<()>();
    ctrlc::set_handler(move || {
        let _ = sender.send(());
    })
    .map_err(|error| {
        WhippletreeError::internal(format!("failed to install signal handler: {error}"))
    })?;
...
fn spawn_detached_daemon(workspace: &Workspace) -> WhippletreeResult<()> {
    let runtime_paths = WorkspaceRuntimePaths::for_workspace(workspace)?;
    ...
    command.spawn().map_err(|error| {
        WhippletreeError::internal(format!("failed to spawn detached daemon: {error}"))
    })?;
    Ok(())
}

fn wait_for_daemon(workspace: &Workspace, timeout: Duration) -> WhippletreeResult<()> {
    ...
    loop {
        ...
        if elapsed_since(start) >= timeout {
            return Err(WhippletreeError::unavailable(format!(
...
fn wait_for_daemon_stop(workspace: &Workspace, timeout: Duration) -> WhippletreeResult<()> {
    let start = SystemTime::now();
    let client = daemon_client(workspace);
...
fn build_task_views(config: &WhippletreeConfig, inspect: Option<&InspectResponse>) -> Vec<TaskView> {
...
fn run_summary(run: &whippletree_core::RunRecord) -> RunSummary {
    RunSummary {
...
fn run_failed(run: &whippletree_core::RunRecord) -> bool {
...
fn read_log_file(path: &Path, tail: Option<usize>) -> WhippletreeResult<LogFileSnapshot> {
...
fn print_appended_log_bytes(label: &str, path: &Path, offset: u64) -> WhippletreeResult<u64> {
...
fn parse_duration(input: &str) -> WhippletreeResult<Duration> {
    ...
    for (suffix, multiplier) in units {
        if let Some(number) = trimmed.strip_suffix(suffix) {
            let value = number.trim().parse::<u64>().map_err(|error| {
                WhippletreeError::invalid_input(format!("invalid duration {input:?}: {error}"))
            })?;
...
fn enum_text<T>(value: &T) -> WhippletreeResult<String>
where
    T: Serialize,
{
    let encoded =
    ...
    encoded
        ...
        .map(str::to_string)
        .ok_or_else(|| WhippletreeError::internal("expected enum to serialize as a JSON string"))
}
...
fn print_data<T>(format: OutputFormat, value: &T, text: &str) -> WhippletreeResult<()>
where
    T: Serialize,
{
    match format {
...
mod tests {
    ...
    use whippletree_core::{resolve_workspace, WorkspaceRuntimePaths};
    ...
    fn runtime_status_commands_accept_json_output_alias() {
        for command in [
            ...
            let cli = Cli::try_parse_from(["whippletree", command, "--json"]).unwrap();
    ...
    fn global_format_json_still_controls_runtime_status_output() {
        let cli = Cli::try_parse_from(["whippletree", "--format", "json", "status"]).unwrap();
    ...
    fn emit_accepts_explicit_event_source() {
        let cli = Cli::try_parse_from(["whippletree", "emit", "hook.example", "--source", "manual"])
            .unwrap();
    ...
    fn emit_accepts_payload_file_and_stdin_sources() {
        ...
        let cli = Cli::try_parse_from(["whippletree", "emit", "hook.example", "--stdin"]).unwrap();
