...
use crate::{AdmissionPolicy, WhippletreeError, WhippletreeResult, SupervisionPolicy};

pub const CONFIG_DIR_NAME: &str = ".whippletree";
...
pub fn discover_workspace(start: impl AsRef<Path>) -> WhippletreeResult<Workspace> {
...
pub fn load_config(path: impl AsRef<Path>) -> WhippletreeResult<WhippletreeConfig> {
...
pub fn load_workspace_config(workspace: &Workspace) -> WhippletreeResult<WhippletreeConfig> {
    load_config(workspace.config_path())
}

fn discover_workspace_from(start: &Path) -> WhippletreeResult<Workspace> {
    ...
    let start_dir = if canonical_start.is_dir() {
        ...
        canonical_start
            .parent()
            .ok_or_else(|| {
                WhippletreeError::invalid_input("workspace discovery requires a directory")
            })?
            .to_path_buf()
    };
    ...
    Err(WhippletreeError::not_found(format!(
...
fn workspace_from_explicit(path: &Path) -> WhippletreeResult<Workspace> {
    ...
    let root = config_path
        ...
        .ok_or_else(|| WhippletreeError::invalid_input("invalid workspace config path"))?
        .to_path_buf();

    Ok(Workspace { root, config_path })
}

fn parse_config(raw: &str) -> WhippletreeResult<WhippletreeConfig> {
    let value: toml::Value = toml::from_str(raw)
        .map_err(|error| WhippletreeError::invalid_input(format!("invalid TOML config: {error}")))?;
    reject_forbidden_blocks(&value)?;

    let parsed: WhippletreeConfigFile = value.try_into().map_err(|error| {
        WhippletreeError::invalid_input(format!("invalid Whippletree config shape: {error}"))
    })?;

    normalize_config(parsed)
}

fn normalize_config(config: WhippletreeConfigFile) -> WhippletreeResult<WhippletreeConfig> {
    ...
    let mut normalized = WhippletreeConfig {
        version: String::new(),
        tasks,
        services,
    };
...
fn normalize_task(task: TaskConfigFile) -> WhippletreeResult<TaskConfig> {
...
fn normalize_service(service: ServiceConfigFile) -> WhippletreeResult<ServiceConfig> {
...
fn normalize_trigger(
    ...
    on: Option<String>,
    settle: Option<String>,
) -> WhippletreeResult<TriggerConfig> {
...
fn normalize_supervision(
    ...
    name: &str,
    supervision: Option<SupervisionPolicyFile>,
) -> WhippletreeResult<SupervisionPolicyConfig> {
...
) -> WhippletreeResult<HealthCheckConfig> {
...
fn normalize_resources(
    ...
    name: &str,
    resources: Option<ResourcePolicyFile>,
) -> WhippletreeResult<ResourcePolicy> {
...
fn config_version(config: &WhippletreeConfig) -> WhippletreeResult<String> {
    let bytes = serde_json::to_vec(&SerializableConfigVersion {
        tasks: &config.tasks,
        services: &config.services,
    })
    .map_err(|error| {
        WhippletreeError::internal(format!("failed to encode config version: {error}"))
    })?;
...
fn validate_name(kind: &str, value: String) -> WhippletreeResult<String> {
...
fn validate_command(kind: &str, name: &str, value: String) -> WhippletreeResult<String> {
    validate_nonempty(format!("{kind} {name} run").as_str(), value)
}

fn validate_nonempty(context: &str, value: String) -> WhippletreeResult<String> {
...
fn validate_cron(task_name: &str, value: String) -> WhippletreeResult<String> {
...
fn normalize_watch_patterns(task_name: &str, patterns: Vec<String>) -> WhippletreeResult<Vec<String>> {
...
fn validate_event_type(task_name: &str, value: String) -> WhippletreeResult<String> {
...
fn validate_duration_literal(field: &str, name: &str, value: String) -> WhippletreeResult<String> {
    ...
    let split_at = value.find(|ch: char| !ch.is_ascii_digit()).ok_or_else(|| {
        WhippletreeError::invalid_input(format!("{field} for {name} must include a unit"))
    })?;
...
fn reject_forbidden_blocks(value: &toml::Value) -> WhippletreeResult<()> {
    let forbidden = [
        (
            "recipe",
...
mod tests {
    ...
    fn discovers_nearest_ancestor_workspace_only() {
        ...
        fs::create_dir_all(root.join(".whippletree")).unwrap();
        fs::create_dir_all(&nested).unwrap();
        fs::write(
            root.join(".whippletree/project.whip"),
            "[[task]]\nname = \"hello\"\nrun = \"echo hi\"\n",
        )
        .unwrap();
    ...
    fn does_not_search_downward_for_workspace() {
        ...
        fs::create_dir_all(child.join(".whippletree")).unwrap();
        fs::write(
            child.join(".whippletree/project.whip"),
            "[[task]]\nname = \"hello\"\nrun = \"echo hi\"\n",
        )
        .unwrap();
        ...
        assert!(error
            .to_string()
            .contains("no .whippletree/project.whip found while searching upward"));
    }
