...
use crate::error::{WhippletreeError, WhippletreeResult};

fn validate_prefixed_ulid(value: &str, prefix: &str) -> WhippletreeResult<()> {
    let ulid = value
        .strip_prefix(prefix)
        .ok_or_else(|| WhippletreeError::invalid_input(format!("expected {prefix}<ulid>")))?;
    Ulid::from_string(ulid)
        .map(|_| ())
        .map_err(|_| WhippletreeError::invalid_input(format!("invalid ULID in {value}")))
}
...
macro_rules! prefixed_id {
    ($name:ident, $prefix:literal) => {
        ...
        impl $name {
            ...
            pub fn parse(value: impl Into<String>) -> WhippletreeResult<Self> {
...
impl WorkspaceId {
    pub fn from_canonical_path(path: &Path) -> WhippletreeResult<Self> {
