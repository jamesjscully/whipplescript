...
use crate::{WhippletreeError, WhippletreeResult, RunId, Workspace, WorkspaceId};

pub const DEFAULT_STATE_HOME_SUFFIX: &str = ".local/state";
pub const WHIPPLETREE_STATE_DIR_NAME: &str = "whippletree";
pub const WORKSPACES_STATE_DIR_NAME: &str = "workspaces";
pub const RUNS_DIR_NAME: &str = "runs";
pub const DATABASE_FILE_NAME: &str = "whippletree.sqlite";
...
impl WorkspaceRuntimePaths {
    pub fn for_workspace(workspace: &Workspace) -> WhippletreeResult<Self> {
    ...
    pub fn for_workspace_with_state_home(
        ...
        let state_root = state_home
            .as_ref()
            .join(WHIPPLETREE_STATE_DIR_NAME)
    ...
    pub fn ensure_state_root(&self) -> WhippletreeResult<()> {
        fs::create_dir_all(&self.state_root)?;
        Ok(())
    }

    pub fn prepare_run_directory(&self, run_id: &RunId) -> WhippletreeResult<RunPaths> {
...
pub fn state_home_from_env() -> WhippletreeResult<PathBuf> {
    ...
    let home = home
        ...
        .ok_or_else(|| {
            WhippletreeError::invalid_input(
                "unable to resolve Whippletree state home: set XDG_STATE_HOME or HOME",
            )
        })?;
...
mod tests {
    ...
    use super::{
        resolve_state_home, WorkspaceRuntimePaths, WHIPPLETREE_STATE_DIR_NAME, DATABASE_FILE_NAME,
    ...
    fn uses_xdg_state_home_when_available() {
        let state_home = resolve_state_home(
            Some(PathBuf::from("/tmp/whippletree-state").into_os_string()),
            None,
        )
        .unwrap();
        assert_eq!(state_home, PathBuf::from("/tmp/whippletree-state"));
    }
    ...
    fn runtime_paths_separate_state_root_from_run_artifacts() {
        ...
        assert!(paths.state_root().components().any(|component| {
            component.as_os_str() == WHIPPLETREE_STATE_DIR_NAME
