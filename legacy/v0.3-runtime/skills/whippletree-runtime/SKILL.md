---
name: whippletree-runtime
description: Use when coding agents need to run, supervise, observe, or coordinate local project automation with Whippletree; configure Whippletree workspaces; write tasks, services, event handlers, locks, or TypeScript SDK integrations; or debug Whippletree-managed runs, logs, triggers, and daemon state.
---

# Whippletree Runtime

Use Whippletree as local runtime plumbing for ordinary programs. Keep the boundary clear:

- Whippletree owns mechanical invocation truth: process start/stop, supervision, events, triggers, runs, logs, locks, waits, and daemon state.
...
Do not introduce workflow DAGs, durable promises, semantic retries, semantic dedupe, agent graphs, or hidden workflow state unless the user explicitly asks for a layer outside Whippletree.

## First Checks

1. Confirm the CLI is available: `whip --help`. If working from source, build it with `cargo build -p whippletree-cli` and use `target/debug/whip`.
2. Find or create the workspace. Whippletree discovers `.whippletree/project.whip` by walking upward, or accepts `--workspace /path/to/workspace`.
...
Use `.whippletree/project.whip` for stable repository behavior.
...
Use dynamic definitions for ephemeral runtime wiring that should not edit `.whippletree/project.whip`.
...
Use exactly one payload source: `--json`, `--payload-file`, or `--stdin`. If no payload is supplied, Whippletree records `{}`.
...
Whippletree records accepted events before routing them. Trigger outcomes are inspectable and can be `started`, `queued`, `coalesced`, `rejected`, or `superseded` depending on admission policy.

Whippletree does not provide durable queues, replay cursors, exactly-once delivery, or semantic deduplication. If the daemon is down, emit fails instead of buffering.

## Runtime Context

Whippletree-managed task and service processes receive context through environment variables including:

- `WHIPPLETREE_RUN_ID`
- `WHIPPLETREE_RUN_DIR`
- `WHIPPLETREE_WORKSPACE_ROOT`
- `WHIPPLETREE_EVENT_TYPE`
- `WHIPPLETREE_EVENT_JSON`
- `WHIPPLETREE_EVENT_PATH`
- `WHIPPLETREE_CORRELATION_ID`

When an Whippletree-managed process calls `whip event emit` or `whip task run`, the CLI forwards mechanical provenance from the environment so records may include `source_run_id`, `parent_event_id`, and `correlation_id`.
...
Use `@whippletree/sdk` when writing Node/TypeScript task or service code. It shells out to the same CLI and reads the same runtime environment; it is not a second runtime.

```ts
import { createWhippletree, emit, getEvent, getRunContext, withLock } from "@whippletree/sdk";

const whippletree = createWhippletree({ workspace: process.cwd() });
...
await withLock("repo:main", async () => {
  console.log(await whippletree.status());
...
5. Whippletree owns timers, event dispatch, run capture, logs, trigger outcomes, waits, and compact inspection.
...
If CLI e2e tests uniformly fail with `No such file or directory` while spawning `whip`, stale Cargo artifacts may contain an old absolute `CARGO_BIN_EXE_whip` path. Run `cargo clean -p whippletree-cli` and retry.

## References

Read these repo files when changing Whippletree itself or when details matter:

- `README.md` for the current user-facing command surface.
- `spec/whippletree-v0.3.md` for the normative v0.3 behavior.
