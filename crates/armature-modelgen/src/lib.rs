...
use armature_workflow::{expr::Expr, ir::State, schema::Schema, WorkflowIr};
...
fn record_output_space(
    fields: &[armature_workflow::schema::Field],
    types: &BTreeMap<String, Schema>,
    depth: usize,
) -> Vec<String> {
...
fn builtin_invariant_coverage_lines(ir: &WorkflowIr) -> Vec<String> {
    ...
    for invariant in &ir.invariants {
        if let armature_workflow::ir::Invariant::Builtin { name, .. } = invariant {
...
fn effect_label_for_step(step: &armature_workflow::ir::Step) -> Option<String> {
...
fn tla_module_name(name: &str) -> String {
    let mut module_name = String::from("Armature_");
...
fn maude_module_name(name: &str) -> String {
    let mut module_name = String::from("ARMATURE-");
    for character in name.chars() {
...
mod tests {
    ...
    fn emits_tla_model_for_minimal_workflow() {
        ...
        assert!(model.contains("---- MODULE Armature_Minimal ----"));
...
"#;
    ...
    fn emits_maude_model_for_minimal_workflow() {
        ...
        assert!(model.contains("mod ARMATURE-MINIMAL is"));
        assert!(model.contains(
            "*** - declaredEffectsOnly: static validation plus generated DeclaredEffectType"
        ));
