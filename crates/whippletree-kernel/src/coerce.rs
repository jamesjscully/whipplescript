...
mod tests {
    ...
    fn real_baml_coerce_endpoint_smoke() {
        let Ok(endpoint) = std::env::var("WHIPPLETREE_BAML_TEST_ENDPOINT") else {
            return;
        };
        let Ok(function_name) = std::env::var("WHIPPLETREE_BAML_TEST_FUNCTION") else {
            return;
        };
        let Ok(arguments_json) = std::env::var("WHIPPLETREE_BAML_TEST_ARGUMENTS_JSON") else {
            return;
        };
        let Ok(output_type) = std::env::var("WHIPPLETREE_BAML_TEST_OUTPUT_TYPE") else {
            return;
        };
