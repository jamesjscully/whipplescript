...
CREATE UNIQUE INDEX runs_effect_terminal_idx
    ON runs(effect_id)
...
VALUES
    ...
    ('loft.evidence', 'Attach structured Whippletree evidence to Loft.', '{}'),
...
VALUES
    ...
    ('profile_repo_reader', 'repo-reader', 'Allow repository reads and agent turns without writes.', 'enforce', '["agent.tell","repo.read","human.ask","baml.coerce","event.emit"]'),
    ('profile_repo_writer', 'repo-writer', 'Allow repository-writing agent workflows.', 'enforce', '["agent.tell","repo.read","repo.write","loft.show","loft.claim","loft.renew","loft.release","loft.note","loft.transition","loft.evidence","loft.resource_intent","loft.complete","loft.fail","human.ask","baml.coerce","event.emit","capability.call"]'),
    ('profile_internet_research', 'internet-research', 'Allow networked research workflows.', 'enforce', '["agent.tell","internet.research","human.ask","baml.coerce","event.emit"]'),
    ('profile_human_review', 'human-review', 'Allow human review requests and answers.', 'enforce', '["human.ask","event.emit"]');
