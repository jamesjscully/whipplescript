...
impl SqliteStore {
    ...
    pub fn commit_rule(&mut self, commit: RuleCommit<'_>) -> StoreResult<StoredEvent> {
        let payload = rule_commit_payload(commit)?;
        let tx = self.connection.transaction()?;
    ...
    pub fn list_instances(&self) -> StoreResult<Vec<InstanceView>> {
        let mut statement = self.connection.prepare(
            r#"
            SELECT instance_id, program_id, version_id, status, input_json, created_at, updated_at
            FROM instances
    ...
    pub fn rebuild_projections(&mut self, instance_id: &str) -> StoreResult<()> {
        ...
        let events = {
            let mut statement = tx.prepare(
                ...
                FROM events
                WHERE instance_id = ?1 AND event_type = 'rule.committed'
                ORDER BY sequence
                "#,
            )?;
            let rows = statement
        ...
        for (event_id, payload_json) in events {
            replay_rule_commit(&tx, instance_id, &event_id, &payload_json)?;
        }
...
fn rule_commit_payload(commit: RuleCommit<'_>) -> StoreResult<String> {
    let facts = commit
        .facts
        .iter()
        .map(|fact| {
