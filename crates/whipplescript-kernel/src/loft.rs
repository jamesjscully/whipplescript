...
mod tests {
    ...
    fn fake_client_returns_claim_value() {
        ...
        let result = client.execute(&LoftEffectRequest {
            ...
            claim_ready: false,
            issue_version: None,
            actor: Some("whipplescript".to_owned()),
    ...
    fn command_args_match_loft_lease_and_status_spec() {
        ...
        let evidence = LoftEffectRequest {
            ...
            target_status: None,
            evidence_json: None,
            evidence_kind: Some("whipplescript.trace".to_owned()),
    ...
    fn loft_submodule_fixture_shapes_are_compatible() {
        ...
        assert!(
            matches!(
                ...
                Some("loft.whipplescript.fixtures.v1")
            ),
        ...
        for fixture in fixtures {
            ...
            let result = decode_success_response(
                &LoftEffectRequest {
                    ...
                    claim_ready: false,
                    issue_version: None,
                    actor: Some("whipplescript-fixture".to_owned()),
                    lease_duration_seconds: Some(1800),
    ...
    fn loft_fixture_dir() -> Option<PathBuf> {
        if let Ok(path) = std::env::var("WHIPPLESCRIPT_LOFT_FIXTURE_DIR") {
            return Some(PathBuf::from(path));
        }

        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
        [
            root.join("vendor/loft/fixtures/whipplescript/v0.1"),
    ...
    fn real_loft_show_smoke() {
        let Ok(issue_id) = std::env::var("WHIPPLESCRIPT_LOFT_TEST_ISSUE") else {
            return;
        };
        let executable =
            std::env::var("WHIPPLESCRIPT_LOFT_CLI").unwrap_or_else(|_| "loft".to_owned());
        ...
        let result = client.execute(&LoftEffectRequest {
            ...
            actor: None,
            lease_duration_seconds: None,
            command_id: "cmd-whipplescript-real-loft-show-smoke".to_owned(),
            note: None,
            target_status: None,
            evidence_json: None,
            evidence_kind: None,
