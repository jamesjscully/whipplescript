...
use crate::{AdmissionPolicy, ArmatureError, ArmatureResult, SupervisionPolicy};

pub const CONFIG_DIR_NAME: &str = ".armature";
pub const CONFIG_FILE_NAME: &str = "armature.toml";
...
pub fn discover_workspace(start: impl AsRef<Path>) -> ArmatureResult<Workspace> {
...
pub fn load_config(path: impl AsRef<Path>) -> ArmatureResult<ArmatureConfig> {
...
pub fn load_workspace_config(workspace: &Workspace) -> ArmatureResult<ArmatureConfig> {
    load_config(workspace.config_path())
}

fn discover_workspace_from(start: &Path) -> ArmatureResult<Workspace> {
    ...
    let start_dir = if canonical_start.is_dir() {
        ...
        canonical_start
            .parent()
            .ok_or_else(|| {
                ArmatureError::invalid_input("workspace discovery requires a directory")
            })?
            .to_path_buf()
    };
    ...
    Err(ArmatureError::not_found(format!(
...
fn workspace_from_explicit(path: &Path) -> ArmatureResult<Workspace> {
    ...
    let root = config_path
        ...
        .ok_or_else(|| ArmatureError::invalid_input("invalid workspace config path"))?
        .to_path_buf();

    Ok(Workspace { root, config_path })
}

fn parse_config(raw: &str) -> ArmatureResult<ArmatureConfig> {
    let value: toml::Value = toml::from_str(raw)
        .map_err(|error| ArmatureError::invalid_input(format!("invalid TOML config: {error}")))?;
    reject_forbidden_blocks(&value)?;

    let parsed: ArmatureConfigFile = value.try_into().map_err(|error| {
        ArmatureError::invalid_input(format!("invalid Armature config shape: {error}"))
    })?;

    normalize_config(parsed)
}

fn normalize_config(config: ArmatureConfigFile) -> ArmatureResult<ArmatureConfig> {
    ...
    let mut normalized = ArmatureConfig {
        version: String::new(),
        tasks,
        services,
    };
...
fn normalize_task(task: TaskConfigFile) -> ArmatureResult<TaskConfig> {
...
fn normalize_service(service: ServiceConfigFile) -> ArmatureResult<ServiceConfig> {
...
fn normalize_supervision(
    ...
    name: &str,
    supervision: Option<SupervisionPolicyFile>,
) -> ArmatureResult<SupervisionPolicyConfig> {
...
) -> ArmatureResult<HealthCheckConfig> {
...
fn config_version(config: &ArmatureConfig) -> ArmatureResult<String> {
    let bytes = serde_json::to_vec(&SerializableConfigVersion {
        tasks: &config.tasks,
        services: &config.services,
    })
    .map_err(|error| {
        ArmatureError::internal(format!("failed to encode config version: {error}"))
    })?;
...
fn validate_name(kind: &str, value: String) -> ArmatureResult<String> {
...
fn validate_command(kind: &str, name: &str, value: String) -> ArmatureResult<String> {
    validate_nonempty(format!("{kind} {name} run").as_str(), value)
}

fn validate_nonempty(context: &str, value: String) -> ArmatureResult<String> {
...
fn validate_cron(task_name: &str, value: String) -> ArmatureResult<String> {
...
fn normalize_watch_patterns(task_name: &str, patterns: Vec<String>) -> ArmatureResult<Vec<String>> {
...
fn validate_event_type(task_name: &str, value: String) -> ArmatureResult<String> {
...
fn validate_duration_literal(field: &str, name: &str, value: String) -> ArmatureResult<String> {
    ...
    let split_at = value.find(|ch: char| !ch.is_ascii_digit()).ok_or_else(|| {
        ArmatureError::invalid_input(format!("{field} for {name} must include a unit"))
    })?;
...
fn reject_forbidden_blocks(value: &toml::Value) -> ArmatureResult<()> {
    let forbidden = [
        (
            "recipe",
        ...
        ("plan", "armature plan is outside the v0.3 config boundary"),
...
mod tests {
    ...
    fn discovers_nearest_ancestor_workspace_only() {
        ...
        fs::create_dir_all(root.join(".armature")).unwrap();
        fs::create_dir_all(&nested).unwrap();
        fs::write(
            root.join(".armature/armature.toml"),
            "[[task]]\nname = \"hello\"\nrun = \"echo hi\"\n",
        )
        .unwrap();
    ...
    fn does_not_search_downward_for_workspace() {
        ...
        fs::create_dir_all(child.join(".armature")).unwrap();
        fs::write(
            child.join(".armature/armature.toml"),
            "[[task]]\nname = \"hello\"\nrun = \"echo hi\"\n",
        )
        .unwrap();
        ...
        assert!(error
            .to_string()
            .contains("no .armature/armature.toml found while searching upward"));
    }
